...
*									     	*
* Run:									     	*
* GoTo_State_Cells -e empty -p ../../data/cells/ -selection 16 -x 0.222467 -y 0.288354 -a 1.451600 -v 0.607500 -z 0.725000 									     	*
*									     	*
*                               J. Savage                                    	*
*                                                                        	*
*                               FI-UNAM 2023                                 	*
...
// it reads the environment file
 ...
 num_polygons_unk = inputs.nn_unk;
// num_polygons_unk = 1;
 ...
 while(flg == 0){
	...
	if(num_polygons_unk > 0){
		...
		if(num_delay_unk > NUM_DELAY_UNK){
                	create_unknown_obstacles(fpw,inputs.radio_robot,dimx,dimy,1,coord_robot.xc,coord_robot.yc);
			num_delay_unk = 1;
		}
		else{
                	 create_unknown_obstacles(fpw,inputs.radio_robot,dimx,dimy,0,coord_robot.xc,coord_robot.yc);
			 num_delay_unk ++;
		}
		// it gets laser range data from unknown obstacles the simulator or the real robot, this function is in ../simulator/simulation.h
        	get_sensor_values_unknown(coord_robot,inputs.theta_sensor,inputs.range_sensor,&observations,inputs.num_sensors,largest_value);
	}
	...
	if(flg_inte_quad == 1){
		inte_quad_obs.sensors[0] = intensity;
		inte_quad_obs.sensors[1] = angle_light;
#ifdef DEBUG
       		printf("inte_quad_obs.sensors[0] %f\n",inte_quad_obs.sensors[0]);
       		printf("inte_quad_obs.sensors[1] %f\n",inte_quad_obs.sensors[1]);
#endif
	

		for(i=0;i<inputs.num_sensors;i++){
		 	inte_quad_obs.sensors[2+i]=observations.sensors[i];
#ifdef DEBUG
       		 	printf("inte_quad_obs.sensors[%d] %f\n",2+i,inte_quad_obs.sensors[2+i]);
#endif
		}
		quantized_obs=quantize_inputs(inte_quad_obs,inputs.num_sensors+2,flg_vq,size_vq,inputs.path);
   	}
	else{
	...
	if(flg_vq == 1) 
		if(flg_inte_quad == 1)
			write_vq_obs_sensor(fpw,quantized_obs,inputs.sensor,inputs.num_sensors+2,inputs.theta_sensor,inputs.range_sensor,size_vq,inputs.path,flg_inte_quad);
		else
			write_vq_obs_sensor(fpw,quantized_obs,inputs.sensor,inputs.num_sensors,inputs.theta_sensor,inputs.range_sensor,size_vq,inputs.path,flg_inte_quad);
	...
	if(training == 1){
		...
		// it saves the sensor data to be used by the VQ; function in /home/biorobotica/robotics/utilities/utilities.h
        	write_sensor_data(fp_sensors,intensity,angle_light,inputs.num_sensors,observations,flg_inte_quad);
	...
	if(selection == 1){
		...
		mov_vector_fsm_engine = state_machine_destination_hmm(quantized_attraction, quantized_intensity, 
				quantized_obs, state, &next_state_fsm_engine,inputs.Mag_Advance,
                                inputs.max_angle, file_hmm, inputs.path, flg_noise,selection,num_bits_vq,1);
//#ifdef DEBUG
...
//#endif
		...
		if(flg_noise==1){
		 ...
		 // It moves the robot to the desire angle and distance, function in ../utilities/utilities.h"
                mvrobot(fpw,DistTheta,&coord_robot);
	    ...
	    mov_vector_destination = go_to_mdp_gen_stalin(inputs,num_obs,quantized_attraction,quantized_intensity,observations);
            ...
            DistTheta.distance=mov_vector_destination.distance;
                ...
                // It calculates the robot's movement using a Meally state machine with the structure input/state generated by an genetic algorithm
                state=next_state_fsm_engine;
		...
		// Meally State Machine
                mov_vector_fsm_engine = state_machine_engine_input_state(quantized_obs, quantized_attraction,
                        quantized_intensity, state, &next_state_fsm_engine,inputs.Mag_Advance,inputs.max_angle,inputs.path,num_bits_vq,1);
                ...
                // It calculates the robot's movement using a Moorestate machine with the structure input/state generated by an genetic algorithm
                state=next_state_fsm_engine;
                // state_machine_engine_input_state in ../state_machines/state_machine_engine.h
		// Moore State Machine
                mov_vector_fsm_engine = state_machine_engine_input_state(quantized_obs, quantized_attraction,
                        quantized_intensity, state, &next_state_fsm_engine,inputs.Mag_Advance,inputs.max_angle,inputs.path,num_bits_vq,0);
        ...
        // It calculates the robot's movement using an Moore/Meally HMMs state machine generated by an genetic algorithm with stochastic outputs
                state=next_state_fsm_engine;

                // state_machine_destination_hmm in file /hmm_robots/state_machine_hmm.h
                mov_vector_fsm_engine = state_machine_destination_hmm(quantized_attraction, quantized_intensity,
                                quantized_obs, state, &next_state_fsm_engine,inputs.Mag_Advance,
                                inputs.max_angle, file_hmm, inputs.path, flg_noise,selection,num_bits_vq,0);
//#ifdef DEBUG
                //printf("movement destination: angle  %f distance %f\n",mov_vector_destination.angle,mov_vector_destination.distance);
//#endif
                ...
                // It calculates the robot's movement using potential fields for a 3D robot
                // potential_fields function is in the following file: ../potentials/potentials.h
                mov_vector_potential_fields=potential_fields_3d(inputs,quantized_attraction,intensity, observations);
...
#ifdef DEBUG
                                printf("The robot rotates to the angle of the light source\n");
#endif
				// It robot first rotates to the light source       
