...
coord get_repulsion_force(Inputs inputs, Raw obs){

	int j;
	coord repulsion_force,dif_vector;
	float x,y,theta_in;
	float mag,tmp,mag3;
	...
	for( j=0; j< num_sensors; j++) {
			...
			if(obs.sensors[j] < threshold){
				...
				dif_vector.xc= - x;
				dif_vector.yc= - y;
				mag=magnitude(dif_vector) + (float) 0.0001;
...
coord get_attraction_force(coord coord_robot,coord coord_destination,float intensity){
	...
	mag= (1/intensity) * magnitude(attraction_force);
	//printf(" 1/intensity %f magnitude(attraction_force) %f mag %f\n",1/intensity,magnitude(attraction_force),mag);

	if(mag> d1){
		// F =(e2 * sx) / magnitud
		attraction_force=multiply_vector_scalar(attraction_force,(float)  -E2);
		attraction_force=divide_vector_scalar(attraction_force,mag);
		//printf("e2 %f\n",E2);

	}
	else{
		// F = e1 * sx
		attraction_force=multiply_vector_scalar(attraction_force,(float)  -e1);
		//printf("e1 %f\n",e1);
...
AdvanceAngle potential_fields(Inputs inputs, int quantized_attraction, float intensity, Raw obs)
{
...
// it reads the constants file the first time this function is called
 ...
 repulsion_vector=get_repulsion_force(inputs,obs);
 ...
 attraction_vector= get_attraction_force(coord_robot,coor_destination,intensity);
#ifdef DEBUG
 ...
 tmp=add_vectors(repulsion_vector,attraction_vector);
#ifdef DEBUG
 ...
 unit_vector=get_unit_vector(pot_vector);
 ...
 get_dist_theta(coord_robot.xc,coord_robot.yc,coord_robot.anglec,next_robot.xc,next_robot.yc,&distance,&theta);
