...
/*
 * Función que ejecuta el control de velocidad dada una posición del robot y una posición deseada
 * Parámetros:
 * float robot_x       Posición X del robot
 * float robot_y       Posición Y del robot
 * float robot_t       Ángulo theta del robot
 * float goal_x        Posición deseada X
 * float goal_y        Posición deseada Y
 * float cruise_speed  Velocidad máxima que alcanzará el robot durante el movimiento
 * bool backwards      Indica si el movimiento se realizará en reversa. 
 * Valor de retorno:
 * geometry_msgs::Twist    Devuelve la acción de control como una velocidad en el espacio definida
                           en un mensaje de tipo Twist.
 */
        ...
        float goal_y, float cruise_speed)
{
    ...
    if(angle_error >   M_PI) angle_error -= 2*M_PI;
    if(angle_error <= -M_PI) angle_error += 2*M_PI;
    bool backwards = angle_error > M_PI/2 || angle_error < -M_PI/2; 
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher  pub_goal_reached     = n.advertise<std_msgs::Bool>      ("/path_goal_reached", 1);                           
    ros::Publisher  pub_cmd_vel          = n.advertise<geometry_msgs::Twist>("/cmd_vel"     , 1);
    ros::Subscriber sub_goalDistance     = n.subscribe("/goal_dist"      , 1, callback_goal_dist);                     
    ros::Subscriber sub_goalDistAngle    = n.subscribe("/goal_dist_angle", 1, callback_goal_dist_angle);          
    ros::Subscriber sub_goalPath         = n.subscribe("/goal_path"      , 1, callback_goal_path);                     
    ros::Subscriber sub_robotStop        = n.subscribe("/stop", 1, callback_robot_stop);               
    ros::Subscriber sub_gollisionRisk    = n.subscribe("/navigation/obs_detector/collision_risk", 10, callback_collision_risk);
    tf::TransformListener tf_listener;
    ros::Rate loop(20);

    float max_cruise_speed = 0.35;
    if(ros::param::has("~max_linear_speed"))
        ros::param::get("~max_linear_speed", max_cruise_speed);
    if(ros::param::has("~max_angular_speed"))
        ros::param::get("~max_angular_speed",max_angular);
    if(ros::param::has("~control_alpha"))
        ros::param::get("~control_alpha", alpha);
    if(ros::param::has("~contrl_beta"))
        ros::param::get("~control_beta", beta);

    std::cout << "Simplemove.->Control parameters: max_linear=" << max_cruise_speed << "  max_angular=" << max_angular << "  alpha=" << alpha << "  beta=" << beta << std::endl;
    ...
    try
    {
        ...
        tf_listener.waitForTransform("map",  "base_footprint",now, ros::Duration(10.0));
        
        tf_listener.lookupTransform("map","base_footprint", now, transform);        
    }
    // catch(...)
    // {
    //     std::cout << "SimpleMove.->Cannot get tranforms map to base footprint... :'(" << std::endl;
    //     return -1;
    // }
    ...
    try
    {
        tf_listener.waitForTransform("odom", "base_footprint", ros::Time(0), ros::Duration(10.0));
        tf_listener.lookupTransform("odom",  "base_footprint", ros::Time(0), transform);
    }
    catch(...)
    {
        std::cout << "SimpleMove.->Cannot get tranforms odom to base footprint... :'(" << std::endl;
        return -1;
    }
        ...
        switch(state)
        {
            ...
            case SM_GOAL_PATH_ACCEL:
                ...
                else
                {
                    ...
                    else if(cruise_speed >= max_cruise_speed)
