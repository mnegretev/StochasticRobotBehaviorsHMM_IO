...
#include "../potentials_3d/potentials_3d.h"
...
int go_to(Inputs inputs)
{
 ...
 Raw inte_quad_obs;
 //int flg_inte_quad = 0;
 int flg_inte_quad = 1; // To include the intensity and angle of the light source in the quantization
 //int flg_inte_quad = 2;
 int num_obs=1;
 float distance1;
 coord coord_robot;
 ...
 while(flg == 0){
	...
	if(training == 1){
		...
		// it saves the sensor data to be used by the VQ; function in /home/biorobotica/robotics/utilities/utilities.h
        	write_sensor_data(fp_sensors,intensity,angle_light,inputs.num_sensors,observations,flg_inte_quad,coord_robot);
...
//#endif
		...
		// Meally State Machine
                ...
                // It calculates the robot's movement using a Moore state machine with the structure input/state generated by an genetic algorithm
                state=next_state_fsm_engine;
                // state_machine_engine_input_state in ../state_machines/state_machine_engine.h
		// Moore State Machine
                mov_vector_fsm_engine = state_machine_engine_input_state(quantized_obs, quantized_attraction,
