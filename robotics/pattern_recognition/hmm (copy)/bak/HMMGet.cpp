...
/* it gets line inputs */
int HMMGet::get_inputs(int argc, char **argv, char *object,int *show,int *num_states,char *path, int *size_vq, char *vq_file)
{
...
int HMMGet::update_hmm_viterbi(char *object,int num_images,int *num_vectors,int **obs, int num_states, char *path, int num_symbols)
{
...
// It saves the transition and initial probabilities
    sprintf(output_file,"%s%s_%d.prb",path,object,num_symbols);
...
int HMMGet::create_initial_hmm(char *object,int num_images,int *size_images,int **obs, int num_states, char *path, int num_symbols)
{
    char output_file[250];
//int *vect,***vectk,*q;
...
//for(k=1;k<60;k++){
    ...
    printf("total output file %s%s_%d.prb\n",path,object,num_centroids_vision);
