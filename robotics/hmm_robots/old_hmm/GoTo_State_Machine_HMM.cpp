...
*									     	*
*  Compile:								     	*
*	make -f Makefile_GoTo_State_Machine_HMM				     	*
*									     	*
*                               J. Savage                                    	*
*                                                                        	*
*                               FI-UNAM 2015                                 	*
...
int go_to(Inputs inputs)
{
 ...
 if(selection == 1){
 	strcpy(file_hmm,"hmm_destination_fsm");
 }
 else if(selection == 2){
 	strcpy(file_hmm,"hmm_destination_fsm");
        }
 ...
 while(flg == 0){
	...
	// It calculates the robot's movement using a probabilistic state machine

	state=next_state_destination;
	printf("state  %d\n",state);
	mov_vector_destination = state_machine_destination_hmm(quantized_attraction, quantized_intensity, quantized_obs, state, &next_state_destination,inputs.Mag_Advance,
				inputs.max_angle, file_hmm, inputs.path, flg_noise,selection);
