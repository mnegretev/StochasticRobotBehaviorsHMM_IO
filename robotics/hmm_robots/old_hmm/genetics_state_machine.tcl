...
global check_dest
 ...
 set num_prb 8
 set num_bits_individuals_prb [expr $num_prb * ( $num_Pi + $num_A + $num_B + $num_C ) ]
 puts "num_bits_individuals_prb $num_bits_individuals_prb"
 ...
 # It checks if a new population is created
 if { $check_population == 1 } {
	...
	if { $prb_out == 1} {
		...
		if { $check_vasconcelos == 0 } {
                	puts "$gen genera standard $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb"
                	catch {exec $gen genera standard $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb  > test.dat 2> error.dat} result
        	} else {
                	puts "$gen genera vasconcelos $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb"
                	catch {exec $gen genera vasconcelos $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb  > test.dat 2> error.dat} result
        	}
 ...
 for {set i 0} {$i< $num_gen} {incr i} {
	...
	# It transform the raw bits to a state machine structure 
        if { $get_st == 1} {
		puts "$transform state_machine $path_observations $num_individuals $num_bits_states $num_bits_outputs $num_inputs $num_bits_individuals $name_pop" 
		catch {exec $transform state_machine $path_observations $num_individuals $num_bits_states $num_bits_outputs $num_inputs $num_bits_individuals $name_pop > test_transform.dat 2> error.dat} result
		puts $result

	} else {

		#it uses the state machine of the best individual
		puts "$command $path_observations$name_pop_0 $path_observations$state_machine"
        	catch {exec $command $path_observations$name_pop_0 $path_observations$state_machine } result
        	puts $result
	}

	
	# It transform the raw bits to a HMM structure 
	if { $prb_out == 1} {
	   puts "$transform state_machine_hmm $path_observations $num_individuals $num_states $num_outputs $num_bits_in $num_symbols $num_bits_individuals_prb $name_pop_prb $num_prb"
    	   catch {exec $transform state_machine_hmm $path_observations $num_individuals $num_states $num_outputs $num_bits_in $num_symbols $num_bits_individuals_prb $name_pop_prb $num_prb > test_transform.dat 2> error.dat} result
	   puts $result
	}
 ...
 for {set j 0} {$j<$num_individuals} {incr j} {
	...
	# It copies the individual to state_machine.txt 
	if { $get_st == 1} { 
		puts "$command $path_observations$name_pop_$j.dat $path_observations$state_machine"
		catch {exec $command $path_observations$name_pop_$j.dat $path_observations$state_machine} result
		puts $result
	}


	if { $prb_out == 1} {
		# It copies the individual's output probabilty to fsm_memory.prb 
        	puts "$command $path_observations$name_pop_prb_$j.dat $path_observations$fsm_memory"
        	catch {exec $command $path_observations$name_pop_prb_$j.dat $path_observations$fsm_memory} result
        	puts $result
	}
	...
	for {set num_pnt 1} {$num_pnt<=$num_pnts} {incr num_pnt} {
		...
		# It executes GoTo
		puts "$program -e $map -x $x -y $y -v $v -z $z -r 2001 -a localhost -w 2005 -s laser -i 2002 -j localhost -k 2006 -l 16 -n 32 -t $angle -f 0 -g 0 -q 0 -m 0  -p $advance -o $random -prb_out $prb_out -path $path_observations"
		catch {exec $program -e $map -x $x -y $y -v $v -z $z -r 2001 -a localhost -w 2005 -s laser -i 2002 -j localhost -k 2006 -l 16 -n 32 -t $angle -f 0 -g 0 -q 0 -m 0  -p $advance -o $random -prb_out $prb_out -path $path_observations > test.dat 2> error.dat} result
		#puts $result
   ...
   if { $get_st == 1} {
     ...
     #It generates a new population for the probabilities
    if { $prb_out == 1} {

   	puts "It generates a new population"
   	if { $check_vasconcelos == 0 } {
        	puts "$gen standard $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb $mutation_rate $crossing_rate $n_best"
        	catch {exec $gen standard $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb $mutation_rate $crossing_rate $n_best} result
   	} else {

        	puts "$gen vasconcelos $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb $mutation_rate $crossing_rate $n_best"
        	catch {exec $gen vasconcelos $path_observations $num_ind $num_bits_individuals_prb $name_pop_prb $mutation_rate $crossing_rate $n_best} result
  ...
  # It transform the raw bits to a state machine structure 
  #puts "$transform state_machine $path_observations $num_individuals $num_bits_states $num_bits_outputs $num_inputs $num_bits_individuals $name_pop"
  #catch {exec $transform state_machine $path_observations $num_individuals $num_bits_states $num_bits_outputs $num_inputs $num_bits_individuals $name_pop > test_transfor_2.dat 2> error.dat} result
  #puts $result

  #if { $prb_out == 1} {
           #puts "$transform state_machine_hmm $path_observations $num_individuals $num_states $num_outputs $num_bits_in $num_inputs $num_bits_individuals_prb $name_pop_prb $num_prb"
           #catch {exec $transform state_machine_hmm $path_observations $num_individuals $num_states $num_outputs $num_bits_in $num_inputs $num_bits_individuals_prb $name_pop_prb $num_prb > test_transform.dat 2> error.dat} result
           #puts $result
 #}
  ...
  if { $prb_out == 1} {
                # It copies the individual's output probabilty to fsm_memory.prb 
                puts "$command $path_observations$name_pop_prb_0 $path_observations$fsm_memory"
                catch {exec $command $path_observations$name_pop_prb_0 $path_observations$fsm_memory} result
                puts $result
  }
  ...
  # It executes GoTo
  puts "$program -e $map -x $x -y $y -v $v -z $z -r 2001 -a localhost -w 2005 -s laser -i 2002 -j localhost -k 2006 -l 16 -n 32 -t $angle -f 0 -g 0 -q 0 -m 0 -p $advance -o $random -prb_out $prb_out -path $path_observations"
  catch {exec $program -e $map -x $x -y $y -v $v -z $z -r 2001 -a localhost -w 2005 -s laser -i 2002 -j localhost -k 2006 -l 16 -n 32 -t $angle -f 0 -g 0 -q 0 -m 0 -p $advance -o $random -prb_out $prb_out -path $path_observations} result
  #puts $result
...
.sim.path insert 0 "/fedora/home/savage/"
