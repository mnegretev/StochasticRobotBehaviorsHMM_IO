...
void fprint_state_stochastic(int index, int num_bits_states, int num_bits_inputs, int size_memory, int num_variables, cromosome *pop, char *file_individual){
  ...
  printf("\n\npop %s\n",file_individual);
  fprintf(fp,"%d %d %d\n",num_bits_states,num_variables,num_bits_inputs);
  printf("%d %d %d\n",num_bits_states,num_variables,num_bits_inputs);
  ...
  for(j=0;j < size_memory; j++){
        ...
        for (i=1;i<=num_variables;i++){
                fprintf(fp,"%f ",pop[index].variables[j][i]);
                printf("%f ",pop[index].variables[j][i]);
        }
        fprintf(fp,"\n");
        printf("\n");
  }
...
void fprint_reactive_output(int index, int num_bits_inputs, int num_bits_output, cromosome *pop, char *file_individual){
  ...
  printf("\n\npop %s\n",file_individual);
  fprintf(fp,"%d %d\n",num_bits_output,num_bits_inputs);
  printf("num_bits_output %d num_bits_inputs %d\n",num_bits_output,num_bits_inputs);

  for(j=0;j < 2<<(num_bits_inputs-1); j++){
        for (i=0;i<num_bits_output;i++){
                fprintf(fp,"%d ",pop[index].output[j][i]);
                printf("%d ",pop[index].output[j][i]);
        }
...
void fprint_reactive_stochastic_output(int index, int num_bits_inputs, int num_var_pr, cromosome_potential *pop, char *file_individual){
  ...
  size_table = 2<<(num_bits_inputs-1);
  ...
  fprintf(fp,"%d %d\n",num_bits_inputs, size_table);
  printf("num_bits_inputs %d size_table %d\n",num_bits_inputs,size_table);

  for(j=1; j <= size_table; j++){
        sum_prb=0.0;
        for(n=1; n <= num_var_pr; n++){
                fprintf(fp,"%f ",pop[index].variables[m]);
                printf(" pop. %d pr[%d][%d] %f\n",index,j,n,pop[index].variables[m]);
...
void fprint_state_output_hmm(int index,cromosome_potential *pop, char *file_individual,int num_bits_inputs, int Meally){
  ...
  if(Meally ==1) limits=num_states*num_symbols;
   else limits=num_states;
...
void fprint_mdp(int index,int num_variables, cromosome_potential *new_pop, char *file_individual){
  ...
  // It prints the rewards 0, 1, 2 and gamma
  for(k=1;k<=4;k++){
...
void transform_reactive_bits(cromosome_binary *old_pop, cromosome *new_pop, int num_bits_inputs, 
										int num_bits_output, int num_individuals){
...
void  transform_pts_hmm(cromosome_binary *old_pop,cromosome_potential *new_pop, int num_variables,
		int num_bits_variables, int num_individuals, int num_states, int num_outputs, int num_symbols, int Meally){
  ...
  // it gets first num_states
	...
	if(Meally == 1) num_total = num_states*num_symbols;
	else num_total = num_states;

        for(nn=1; nn <= num_total; nn++){
...
void transform_in(cromosome_binary *old_pop, cromosome_in *new_pop, int num_bits_states, int num_bits_inputs, int num_bits_output, int num_individuals){
  ...
  for(k=0;k<num_individuals;k++){
    i=0;
    printf("Individuals %d \n",k);
    for(j=0;j < (2<<(num_bits_states-1)); j++){
	// It gets the test_1 bits
	for (l=0;l<num_bits_inputs;l++){
                new_pop[k].test_1[j][l]=old_pop[k].bits[i];
                printf("%d ",new_pop[k].test_1[j][l]);
                i++;
        }
	// It gets the test_2 bits
	for (l=0;l<num_bits_inputs;l++){
                new_pop[k].test_2[j][l]=old_pop[k].bits[i];
                printf("%d ",new_pop[k].test_2[j][l]);
                i++;
        }
	// It gets the false state bits
        for (l=0;l<num_bits_states;l++){
                new_pop[k].state_false[j][l]=old_pop[k].bits[i];
                printf("%d ",new_pop[k].state_false[j][l]);
                i++;
        }
	// It gets the true state bits
        for (l=0;l<num_bits_states;l++){
                new_pop[k].state_true[j][l]=old_pop[k].bits[i];
                printf("%d ",new_pop[k].state_true[j][l]);
                i++;
        }
	// It gets the false output bits
        for (l=0;l<num_bits_output;l++){
                new_pop[k].output_false[j][l]=old_pop[k].bits[i];
                printf("%d ",new_pop[k].output_false[j][l]);
                i++;
        }
	// It gets the true output bits
        for (l=0;l<num_bits_output;l++){
                new_pop[k].output_true[j][l]=old_pop[k].bits[i];
                printf("%d ",new_pop[k].output_true[j][l]);
                i++;
        }
...
void fprint_state_output_in(int index, int num_bits_states, int num_bits_inputs, int num_bits_output, cromosome_in *pop, char *file_individual){
  ...
  printf("\n\npop %s\n",file_individual);
  fprintf(fp,"%d %d %d\n",num_bits_states,num_bits_inputs,num_bits_output);
  printf("%d %d %d\n",num_bits_states,num_bits_inputs,num_bits_output);

  for(j=0;j < (2<<(num_bits_states-1)); j++){
	for (i=0;i<num_bits_inputs;i++){
                fprintf(fp,"%d ",pop[index].test_1[j][i]);
                printf("%d ",pop[index].test_1[j][i]);
        }
	for (i=0;i<num_bits_inputs;i++){
                fprintf(fp,"%d ",pop[index].test_2[j][i]);
                printf("%d ",pop[index].test_2[j][i]);
        }

        for (i=0;i<num_bits_states;i++){
                fprintf(fp,"%d ",pop[index].state_false[j][i]);
                printf("%d ",pop[index].state_false[j][i]);
        }
	for (i=0;i<num_bits_states;i++){
                fprintf(fp,"%d ",pop[index].state_true[j][i]);
                printf("%d ",pop[index].state_true[j][i]);
        }
        for (i=0;i<num_bits_output;i++){
                fprintf(fp,"%d ",pop[index].output_false[j][i]);
                printf("%d ",pop[index].output_false[j][i]);
        }
	for (i=0;i<num_bits_output;i++){
                fprintf(fp,"%d ",pop[index].output_true[j][i]);
                printf("%d ",pop[index].output_true[j][i]);
        }
        fprintf(fp,"\n");
        printf("\n");
  }
...
void transform_in_state_machine(char *args[],int nargc){
  ...
  char file_name[300],file_individual[300],file_name_old[300];
  int i,j,k;
  cromosome_in new_pop[MAX_NUMBER_IND];
  cromosome_binary old_pop[MAX_NUMBER_IND];
  int num_best=NUM_BEST;
  float num_best_factor;
  ...
  for(i=0;i< num_individuals;i++){
        new_pop[i].test_1 =  (int **) alloc_matrix_int((2<<(num_bits_states+1)),num_bits_inputs);
        new_pop[i].test_2 =  (int **) alloc_matrix_int((2<<(num_bits_states+1)),num_bits_inputs);
        new_pop[i].state_false =  (int **) alloc_matrix_int((2<<(num_bits_states+1)),num_bits_states);
        new_pop[i].state_true =  (int **) alloc_matrix_int((2<<(num_bits_states+1)),num_bits_states);
        new_pop[i].output_false =  (int **) alloc_matrix_int((2<<(num_bits_states+1)),num_bits_output);
        new_pop[i].output_true =  (int **) alloc_matrix_int((2<<(num_bits_states+1)),num_bits_output);
  }
  ...
  // it transforms the bits to an structure that the state machine uses  
  transform_in(old_pop,new_pop,num_bits_states,num_bits_inputs,num_bits_output,num_individuals);

  for(i=0;i<num_individuals;i++){
        // it saves the new population
        sprintf(file_individual,"%s_%d.dat",file_name,i);
        fprint_state_output_in(i,num_bits_states,num_bits_inputs,num_bits_output,new_pop,file_individual);
...
void transform_bits_state_machine_stochastic(cromosome_binary *old_pop, cromosome *new_pop, int num_bits_states, int num_bits_inputs, int num_bits_variables, 
				int num_individuals, int num_prb, int size_memory){
  ...
  for(k=0;k<num_individuals;k++){
	...
	// it gets the components of the stochastic outputs
        ...
        ini=m-1;
         ...
         for(n=1; n <= num_prb; n++){
                ...
                for(jj=0;jj < num_bits_variables; jj++){
                        bits[jj]=old_pop[k].bits[i];
                        //printf("%d",bits[jj]);
                        i++;
                }

                var=0;
                for(jj=num_bits_variables-1,l=0;jj >= 1; jj--,l++){
                        var=bits[jj]*(2 << l)+var;
                }

                var= var  + 0.001;
                sum=sum+var;
                new_pop[k].variables[j][n]=var;
                //printf(" pop. %d var[%d][%d] %f\n ",k,j,m,new_pop[k].variables[j][m]);
                m++;
         }
         ...
         // normalization
         for(n=1; n <= num_prb; n++){
                new_pop[k].variables[j][n]=new_pop[k].variables[j][n]/sum;
                sum_prb= sum_prb + new_pop[k].variables[j][n];
                //printf(" %f",new_pop[k].variables[j][n]);
...
void transform_state_machine_stochastic(char *args[],int nargc){
  ...
  num_bits_output=atoi(args[5]);
  num_bits_inputs=atoi(args[6]);
  num_bits_variables=atoi(args[7]);
  strcpy(file_name,PATH);
  strcpy(file_name_old,PATH);
  strcat(file_name_old,"old_");
  strcat(file_name,args[8]);
  ...
  num_states = 2<<(num_bits_states-1);
  num_variables = 2<<(num_bits_output-1);
  size_memory_states = 2<< (num_bits_states+num_bits_inputs-1);
  printf(" num_bits_states %d num_bits_inputs %d\n",num_bits_states,num_bits_inputs);
  printf(" num_bits_output %d num_variables %d num_states %d\n",num_bits_output,num_variables,num_states);
  printf(" num_bits_variables %d\n",num_bits_variables);
  printf("size_memory_states %d num_variables_prb %d \n",size_memory_states,num_variables);

  num_bits_individuals=size_memory_states*(num_bits_states + num_bits_variables*num_variables);
  ...
  for(i=0;i< num_individuals;i++){
        new_pop[i].state =  (int **) alloc_matrix_int(size_memory_states+1,num_bits_states+1);
        new_pop[i].variables =  (float **) alloc_matrix_float(size_memory_states+1,num_variables+1);
  }
  ...
  // it transforms the bits to an structure that the state machine with stochastic outputs uses  
  transform_bits_state_machine_stochastic(old_pop,new_pop,num_bits_states,num_bits_inputs,num_bits_variables,num_individuals,num_variables,
			size_memory_states);
  ...
  for(i=0;i<num_individuals;i++){
        // it saves the new population
        sprintf(file_individual,"%s_%d.dat",file_name,i);
        fprint_state_stochastic(i,num_bits_states,num_bits_inputs,size_memory_states,num_variables,new_pop,file_individual);
...
void transform_reactive(char *args[],int nargc){

  int num_individuals,num_bits_output,num_bits_inputs;
  int num_bits_individuals;
  FILE *fp;
  char file_name[300],file_individual[300],file_name_old[300];
  int i,j,k;
  ...
  num_bits_individuals=(2<<(num_bits_inputs-1))*num_bits_output;
  ...
  for(i=0;i< num_individuals;i++){
        //new_pop[i].state =  (int **) alloc_matrix_int((2<<(num_bits_states+num_bits_inputs+1)),num_bits_states);
        new_pop[i].output =  (int **) alloc_matrix_int((2<<(num_bits_inputs+1)),num_bits_output);
  }
  ...
  // it transforms the bits to an structure that the reactive behavior uses  
  transform_reactive_bits(old_pop,new_pop,num_bits_inputs,num_bits_output,num_individuals);

  for(i=0;i<num_individuals;i++){
        // it saves the new population
        sprintf(file_individual,"%s_%d.dat",file_name,i);
        fprint_reactive_output(i,num_bits_inputs,num_bits_output,new_pop,file_individual);
...
void transform_state_machine_hmm(char *args[],int nargc){
  ...
  if(Meally == 1){
  	...
  	num_variables= num_states * (1 + num_states + num_symbols + num_outputs);
  }
  ...
  transform_pts_hmm(old_pop,new_pop,num_variables,num_bits_variables,num_individuals,
                        num_states,num_outputs,num_symbols,Meally);

  for(i=0;i<num_individuals;i++){
        // it saves the new population
        sprintf(file_individual,"%s_%d.dat",file_name_hmm,i);
        fprint_state_output_hmm(i,new_pop,file_individual,num_bits_inputs,Meally);
  }
...
void transform_reactive_stochastic_bits(cromosome_binary *old_pop, cromosome_potential *new_pop, int size_table, int num_prb,
                                                                                int num_bits_variables, int num_individuals){
  ...
  for(k=0;k<num_individuals;k++){
     ...
     // it gets the components of the stochastic table
...
void transform_reactive_stochastic(char *args[],int nargc){
  ...
  // transform_gen reactive_stochastic ../observations/ 100 3 8 avoid
  ...
  num_prb = 2<<(num_bits_output-1);
  size_table = 2<<(num_bits_inputs-1);
  printf("num_prb %d size_table %d\n",num_prb,size_table);

  num_bits_individuals=size_table*num_prb*num_bits_variables;
  ...
  num_variables= size_table*num_prb*num_individuals;
  printf("Num. variables %d num.bits_individuals %d\n",num_variables,num_bits_individuals);
  for(i=0;i< num_individuals;i++){
        new_pop[i].variables =  (float *) alloc_float(num_variables +1);
  }
  ...
  // it transforms the bits to an structure that the reactive behavior uses  
  transform_reactive_stochastic_bits(old_pop,new_pop,size_table,num_prb,num_bits_variables,num_individuals);

  for(i=0;i<num_individuals;i++){
        // it saves the new population
        sprintf(file_individual,"%s_%d.dat",file_name,i);
        fprint_reactive_stochastic_output(i,num_bits_inputs,num_prb,new_pop,file_individual);
...
void transform_mdp_bits(cromosome_binary *old_pop, cromosome_potential *new_pop, int size_table, int num_prb, int num_bits_variables, int num_individuals, int num_bits_fractions){
  ...
  for(k=0;k<num_individuals;k++){
        ...
        // It forms the rewards 0, 1, 2 and gamma
        for(n=1; n <= 4; n++){
                ...
                new_pop[k].variables[m]=var/frac2;
                printf(" pop. %d var[%d] %f\n ",k,m,new_pop[k].variables[m]);
                m++;
      }
...
void transform_mdp(char *args[],int nargc){
  ...
  transform_mdp_bits(old_pop,new_pop,NUM_MDP_PRB,ROW_PRB*CLM_PRB,num_bits_variables,num_individuals,num_bits_fractions);
