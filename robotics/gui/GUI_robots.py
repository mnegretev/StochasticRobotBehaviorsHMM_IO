...
#HERE
	...
	def initial(self):
		...
		# Plot Training button
                self.TrainingButton = tk.Button(self.topLevelWindow, width = 20, text = 'Training VQ', bg = 'green', activebackground = 'green', command = self.toggleTraining)
                self.countEvaluate = 0
		# Plot Robot button
		...
		self.path.insert ( 0, '../test_data_8/' )
		PATH = self.path.get()
      		# World's File entry 
...
#here
	...
	for k in range(total_num_generations, num_generations+total_num_generations):
	  ...
	  # it evaluates the new HMM in the environments
          #indi_HMM_Viterbi=self.evaluate_worlds(File_Name_cpy,num_worlds,0,0)
	...
	if selection == str(6):
		...
		command = robot_command + origin + destination + " -e " + File_Name + " -p " + PATH  + " -steps " + number_steps + " -noise " + str(flg_noise) + " > test.dat "
