# GUI_robots_test.py 
'''
@author: Jesus Savage, UNAM-FI, 11-2014-2017
'''
...
#HERE
	...
	def callback_mouse_3(event):
		...
		if flg_execute == 1:
			...
			str_evaluation = ("%5.2f" % partial_evaluation).strip()
                        self.Evaluation.delete(0, END)
                        self.Evaluation.insert(0,str_evaluation)
	...
	def initial(self):
		...
		# Plot Evaluate button
                #self.EvaluateButton = tk.Button(self.topLevelWindow, width = 20, text = 'TEST BEST INDIVIDUALS', bg = 'green', activebackground = 'green', command = self.toggleEvaluateRobot)
                self.EvaluateButton = tk.Button(self.topLevelWindow, width = 20, text = 'TEST BEST INDIVIDUALS', bg = 'green', activebackground = 'green', command = self.toggleEvaluatePerformanceRobot)
		...
		# Plot FSM button
        	self.FSMButton = tk.Button(self.topLevelWindow, width = 15, text = 'FSM Human', bg = 'red', activebackground = 'red', command = self.FSM)
		self.countFSM = 0

		# Plot HMM button
        	self.HMMButton = tk.Button(self.topLevelWindow, width = 15, text = 'HMM Human', bg = 'green', activebackground = 'green', command = self.HMM)
		self.countHMM = 0

		# Plot HMM Viterbi button
                self.HMMViterbiButton = tk.Button(self.topLevelWindow, width = 15, text = 'HMM Viterbi', bg = 'green', activebackground = 'green', command = self.HMMViterbi)
                self.countHMM_Viterbi = 0

		# Plot HMM Best button
                self.HMMBestButton = tk.Button(self.topLevelWindow, width = 15, text = 'HMM Gen.', bg = 'green', activebackground = 'green', command = self.HMMBest)
                self.countHMM_Best = 0

                # Plot FSM Best button
                self.FSMBestButton = tk.Button(self.topLevelWindow, width = 15, text = 'FSM Gen.', bg = 'green', activebackground = 'green', command = self.FSMBest)
                self.countHMM_Best = 0

                # Plot NN Best button
                self.NNBestButton = tk.Button(self.topLevelWindow, width = 15, text = 'NN Gen.', bg = 'green', activebackground = 'green', command = self.NNBest)
                self.countHMM_Best = 0

                # Plot MDP Best button
                self.MDPBestButton = tk.Button(self.topLevelWindow, width = 15, text = 'MDP', bg = 'green', activebackground = 'green', command = self.MDPBest)
                self.countHMM_Best = 0

		# Plot REACTIVE button
                self.REACTIVEBestButton = tk.Button(self.topLevelWindow, width = 15, text = 'Deterministic REACTIVE', bg = 'green', activebackground = 'green', command = self.REACTIVEBest)
                self.countHMM_Best = 0

		# Plot REACTIVE button
                self.PRREACTIVEBestButton = tk.Button(self.topLevelWindow, width = 15, text = 'Probabilistic REACTIVE', bg = 'green', activebackground = 'green', command = self.PRREACTIVEBest)
                self.countHMM_Best = 0
		...
		self.path.insert ( 0, '../test_data_4/' )
		PATH = self.path.get()
      		# World's File entry 
		...
		# Evaluation
		self.label_Evaluation = tk.Label(self.topLevelWindow,text =  "Evaluation")
                self.Evaluation = tk.Entry(self.topLevelWindow, width = 8, foreground='white',background='black')
                self.Evaluation.insert ( 0, '0.0' )
                evaluation = self.Evaluation.get()
...
#--------------------------- G R I D S ------------------------------------------------------------- 
		...
		self.EvaluateButton.grid({'row':0, 'column': 5})
		...
		self.label_Evaluation.grid({'row':8, 'column': 4})
                self.Evaluation.grid({'row':8, 'column': 5})
        	self.FSMButton.grid({'row':9, 'column': 0})
        	self.HMMButton.grid({'row':9, 'column': 1})
        	self.HMMViterbiButton.grid({'row':9, 'column': 2})
        	self.HMMBestButton.grid({'row':9, 'column': 3})
        	self.FSMBestButton.grid({'row':9, 'column': 4})
        	self.NNBestButton.grid({'row':9, 'column': 5})
        	self.MDPBestButton.grid({'row':9, 'column': 6})
        	self.REACTIVEBestButton.grid({'row':7, 'column': 6})
        	self.PRREACTIVEBestButton.grid({'row':6, 'column': 6})
...
#___________________________________________________________________________________________________ 
	...
	oval = C.create_oval(X1,Y1,X2,Y2, outline="black",fill="purple", width=1)
	#X1 = X 
	#Y1 = Y
...
#------------------------------------------------------------------------------------------
    ...
    def readResultFile(self,File_Results,File_Constants):
		...
		def __init__(self, File_Constants):
	      ...
	      print "theshold angle ",str(threshold_angle)
	      ...
	      #if num_backwards == 1:
	      	#evaluation = Ct.K1*(number_steps_total - num_steps)*dif_o + Ct.K2*(number_steps_total - num_steps)*(dif_o_d-dif_d) + Ct.K3*dif_o_d/num_steps + Ct.K5*(number_steps_total - num_steps)/num_turns + Ct.K6*(number_steps_total - num_steps)/num_stops + Ct.K7*num_advance*(number_steps_total - num_steps)
	      #else:
...
# Functions that handle the buttons
    ...
    from evaluate_performance import toggleEvaluatePerformanceRobot 
...
#here
	...
	self.FSMButton['bg'] = 'red'
       	self.FSMButton['activebackground'] = 'red'
	self.HMMButton['bg'] = 'green'
       	self.HMMButton['activebackground'] = 'green'
	self.HMMViterbiButton['bg'] = 'green'
       	self.HMMViterbiButton['activebackground'] = 'green'
        self.HMMBestButton['bg'] = 'green'
        self.HMMBestButton['activebackground'] = 'green'
        self.FSMBestButton['bg'] = 'green'
        self.FSMBestButton['activebackground'] = 'green'
        self.NNBestButton['bg'] = 'green'
        self.NNBestButton['activebackground'] = 'green'
        self.MDPBestButton['bg'] = 'green'
        self.MDPBestButton['activebackground'] = 'green'
        self.REACTIVEBestButton['bg'] = 'green'
        self.REACTIVEBestButton['activebackground'] = 'green'
        self.PRREACTIVEBestButton['bg'] = 'green'
        self.PRREACTIVEBestButton['activebackground'] = 'green'
	PATH = self.path.get()
        # It evaluates the human state machine
        self.robot_command.delete(0, END)
        self.robot_command.insert ( 0,"../motion_planner/GoTo_State_Machine")
	...
	self.FSMButton['bg'] = 'green'
       	self.FSMButton['activebackground'] = 'green'
	self.HMMButton['bg'] = 'red'
       	self.HMMButton['activebackground'] = 'red'
	self.HMMViterbiButton['bg'] = 'green'
       	self.HMMViterbiButton['activebackground'] = 'green'
        self.HMMBestButton['bg'] = 'green'
        self.HMMBestButton['activebackground'] = 'green'
	...
	PATH = self.path.get()
        ...
        # It evaluates the human HMM state machine
	# It copies the human created probability file to the probability file of the state machine
        self.robot_command.delete(0, END)
        self.robot_command.insert ( 0,"../hmm_robots/GoTo_State_Machine_HMM")
        command = "cp " + PATH + "hmm_destination_avoidance_fsm.000 " + PATH + "hmm_destination_avoidance_fsm.prb"
	print "cp HMM command: ", command
        status = os.system(command)
        C.update_idletasks()
	...
	self.HMMViterbiButton['bg'] = 'red'
       	self.HMMViterbiButton['activebackground'] = 'red'
        self.FSMButton['bg'] = 'green'
        self.FSMButton['activebackground'] = 'green'
        self.HMMButton['bg'] = 'green'
        self.HMMButton['activebackground'] = 'green'
    ...
    def HMMBest(self):
        ...
        self.HMMBestButton['bg'] = 'red'
        self.HMMBestButton['activebackground'] = 'red'
        self.HMMViterbiButton['bg'] = 'green'
        self.HMMViterbiButton['activebackground'] = 'green'
        self.FSMButton['bg'] = 'green'
        self.FSMButton['activebackground'] = 'green'
    ...
    def FSMBest(self):
        ...
        self.FSMBestButton['bg'] = 'red'
        self.FSMBestButton['activebackground'] = 'red'
        self.FSMButton['bg'] = 'green'
        self.FSMButton['activebackground'] = 'green'
        self.HMMButton['bg'] = 'green'
        self.HMMButton['activebackground'] = 'green'
	...
	global PATH
        ...
        self.NNBestButton['bg'] = 'green'
        self.NNBestButton['activebackground'] = 'green'
        self.MDPBestButton['bg'] = 'red'
        self.MDPBestButton['activebackground'] = 'red'
        self.REACTIVEBestButton['bg'] = 'green'
        self.REACTIVEBestButton['activebackground'] = 'green'
        self.PRREACTIVEBestButton['bg'] = 'green'
        self.PRREACTIVEBestButton['activebackground'] = 'green'
    ...
    def REACTIVEBest(self):
        ...
        self.MDPBestButton['bg'] = 'green'
        self.MDPBestButton['activebackground'] = 'green'
        self.REACTIVEBestButton['bg'] = 'red'
        self.REACTIVEBestButton['activebackground'] = 'red'
        self.PRREACTIVEBestButton['bg'] = 'green'
        self.PRREACTIVEBestButton['activebackground'] = 'green'
        PATH = self.path.get()
        # It evaluates the mdp
        self.robot_command.delete(0, END)
        self.robot_command.insert ( 0,"../reactive_navigation/GoTo_Reactive_Navigation")
        self.selection.delete(0, END)
    ...
    def PRREACTIVEBest(self):
        ...
        self.REACTIVEBestButton['bg'] = 'green'
        self.REACTIVEBestButton['activebackground'] = 'green'
        self.PRREACTIVEBestButton['bg'] = 'red'
        self.PRREACTIVEBestButton['activebackground'] = 'red'
        PATH = self.path.get()
        # It evaluates the mdp
        self.robot_command.delete(0, END)
        self.robot_command.insert ( 0,"../reactive_navigation/GoTo_Reactive_Navigation")
        self.selection.delete(0, END)
        self.selection.insert ( 0, '2' )
        C.update_idletasks()
        # it copies the genetics best state machine
        #command = "cp " + PATH + "best_fsm.txt " + PATH + "state_machine_mem.txt"
        #print "cp Best FSM command: ", command
    ...
    def NNBest(self):
        ...
        self.NNBestButton['bg'] = 'red'
        self.NNBestButton['activebackground'] = 'red'
        self.FSMBestButton['bg'] = 'green'
        self.FSMBestButton['activebackground'] = 'green'
        self.FSMButton['bg'] = 'green'
        self.FSMButton['activebackground'] = 'green'
	...
	robot_cmnd = self.robot_command.get()
	...
	command = robot_cmnd + origin + destination + rest + " > test.dat "
	#here
	#print "Command plotExecute robot: ", command
	status = os.system(command)
