...
import os
import numpy as np
from initial_genetics_NN import *
import time
...
# Functions that handle the buttons
	...
	if num_behavior == 1:
		...
		num_bits_memory_cells = num_bits_states + num_outputs*num_bits_variables;
		...
		num_bits_inputs = num_bits_intensity + num_bits_symbols + num_bits_angle_light
		num_memory_cells = 2 << (num_bits_inputs-1)
        	num_bits_individuals = num_memory_cells * num_outputs * num_bits_variables
...
#here transform
	  ...
	  if num_behavior == 1:
	  	...
	  	# ../genetics/transform_gen state_machine_stochastic ../test_data_14/ 100 4 3 8 8 avoid_fsm_st
          	...
          	command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_states) + " " + str(num_bits_outputs) + " " + str(num_bits_inputs) + " " + str(num_bits_variables) + " " + population_name + " > " + PATH + "transform_" + BEHAVIOR + ".dat"
		...
		command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_variables) + " " + str(num_states) + " " + str(num_symbols) + " " + str(num_outputs) + " " + population_name + " > " + PATH + "transform_hmm.dat"
		...
		# It transform the binary population file to a REACTIVE population files
	  	...
	  	# It transform the binary population file to a REACTIVE stochastic population files
          	...
          	command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_outputs) + " " + str(num_bits_inputs) + " " +  population_name + " " + str(num_bits_variables) + " > " + PATH + "transform_reactive_stochastic.dat"
	  ...
	  for l in range(0, num_individuals):
	     ...
	     # it evaluates the Behaviors
             self.robot_command.delete(0, END)
             self.robot_command.insert ( 0,"../motion_planner/GoTo_State_Machine_Stalin")
             self.selection.delete(0, END)
