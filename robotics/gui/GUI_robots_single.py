# GUI_robots_single.py 
'''
@author: Jesus Savage, UNAM-FI, 11-2020
'''
...
#HERE
	...
	def initial(self):
		...
		self.path.insert ( 0, '../test_data_14/' )
		PATH = self.path.get()
      		# World's File entry 
		...
		self.number_steps.insert ( 0, '200' )
...
#------------------------------------------------------------------------------------------
    ...
    def readResultFile(self,File_Results,File_Constants,num):
		...
		def __init__(self, File_Constants):
	      ...
	      command = "../Dijkstra/Dijkstra -x " + str(x) + " -y " + str(y) + " -v " + str(xd) + " -z " + str(yd) + " -p " + PATH + " -e " + File_Name + " > rslt.dat "
...
#here vq
	# FSM stochastic output genetics' variables
	...
	num_bits_variables = 2
	num_bits_inputs = num_bits_intensity + num_bits_symbols + num_bits_angle_light
	num_bits_memory_cells = num_bits_states + num_outputs_fsm*num_bits_variables;
	num_memory_cells = 2 << (num_bits_states + num_bits_inputs-1)
	#print "Num. Locationts ", str(num_memory_cells) 
  	num_bits_individuals_fsm_st = num_memory_cells * num_bits_memory_cells;
	population_name_fsm_st = "avoid_fsm_st"
	...
	FILE_fitness_average_fsm_st = PATH + 'fitness_average_fsm_st.dat'
       	#print 'File_fitness_average_fsm_st ', FILE_fitness_average_fsm_st
	FILE_fitness_worst_fsm_st = PATH + 'fitness_worst_fsm_st.dat'
       	#print 'File_fitness_worst_fsm_st ', FILE_fitness_worst_fsm_st
	FILE_fitness_best_fsm_st = PATH + 'fitness_best_fsm_st.dat'
       	#print 'File_fitness_best_fsm_st ', FILE_fitness_best_fsm_st
...
#here
		# it generates the first population for the stochastic FSM individuals
		genetics_command = "../genetics/genetics_general genera vasconcelos "
		command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_individuals_fsm_st) + " " + population_name_fsm_st + " > " + PATH + "genetics_general_FSM_st.dat"
		#command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_individuals_fsm_st) + " " + population_name_fsm_st
        	print "First generation command genetics FSM_st: ", command
        	status = os.system(command)
		...
		# FSM_st Files
		command = "rm " + FILE_fitness_average_fsm_st
                #print "Command delete: ", command
                status = os.system(command)
                command = "rm " + FILE_fitness_worst_fsm_st
                #print "Command delete: ", command
                status = os.system(command)
                command = "rm " + FILE_fitness_best_fsm_st
                #print "Command delete: ", command
          	status = os.system(command)
	...
	for k in range(total_num_generations, num_generations+total_num_generations):
	  ...
	  fit_average_fsm_st = 0.0
	  fit_worst_fsm_st = 1000000.0
	  fit_best_fsm_st = 0.0


  	  # It creates the FSM fitness file
          FILE_fitness_fsm_st = PATH + 'fitness_fsm_st.dat'
          file_fit_fsm_st = open(FILE_fitness_fsm_st,'w')
          #print 'File_fitness_fsm_st ', FILE_fitness_fsm_st


	  # It transform the binary population file to a FSM_st population files
	  ...
	  # ../genetics/transform_gen state_machine_stochastic ../test_data_14/ 100 4 3 8 8 avoid_fsm_st
          genetics_command = "../genetics/transform_gen state_machine_stochastic "
          command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_states) + " " + str(num_bits_outputs) + " " + str(num_bits_inputs) + " " + str(num_bits_variables) + " " + population_name_fsm_st + " > " + PATH + "transform_fsm_st.dat"
          #command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_states) + " " + str(num_bits_outputs) + " " + str(num_bits_inputs) + " " + str(num_bits_variables) + " " + population_name_fsm_st
#here
          print "Command transform FSM_st: ", command
          status = os.system(command)
	  ...
	  for l in range(0, num_individuals):
	     ...
	     # cp avoid_99.dat state_machine_mem_stochastic.txt
             command = "cp " + PATH + population_name_fsm_st + "_" + str(l) + ".dat " + PATH + "state_machine_mem_stochastic.txt"
             #print "cp command: ", command
             status = os.system(command)
             indi_evaluation=self.evaluate_worlds(File_Name_cpy,num_worlds,0,0)
             value = str(indi_evaluation) + '\n'
             file_fit_fsm_st.write(value)
             #print "Evaluation Gen. ",str(k)," ind. FSM_st ",str(l)," = ",str(indi_evaluation)

             fit_average_fsm_st = fit_average_fsm_st + indi_evaluation
             if(indi_evaluation > fit_best_fsm_st):
                 fit_best_fsm_st = indi_evaluation
                 index_best_fsm_st = l

               	 # it saves the information of the best individual: environment, the run and the genoma
		 for num_wrlds in range(1, num_worlds+1):
                 	command = "cp " + PATH + File_Name_cpy + "_"+ str(num_wrlds) + '.wrl '  + PATH + "best_fsm_st_" + str(num_wrlds) + "_" + str(k) + ".wrl"
                 	status = os.system(command)
                 	#print "cp command best FSM world: ", command
                 	command = "cp " + PATH + File_Name_cpy + "_"+ str(num_wrlds) + '.raw '  + PATH + "best_fsm_st_" + str(num_wrlds) + "_" + str(k) + ".raw"
                 	status = os.system(command)
                 	#print "cp command best  FSM raw: ", command

                 command = "cp " + PATH + "state_machine_mem_stochastic.txt " + PATH + "best_fsm_st_" + str(k) + ".txt"
                 #print "cp command best_FSM_st behaviour: ", command
                 status = os.system(command)

             if(indi_evaluation < fit_worst_fsm_st):
                 fit_worst_fsm_st = indi_evaluation
                 index_worst_fsm_st = l
                 # it saves the information of the worst individual: environment, the run and the genoma
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.wrl '  + PATH + "worst_fsm_st_" + str(k) + ".wrl"
                 #print "cp command worst_FSM_st world: ", command
                 #status = os.system(command)
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.raw '  + PATH + "worst_fsm_st_" + str(k) + ".raw"
                 #print "cp command worst raw: ", command
                 status = os.system(command)
                 command = "cp " + PATH + "state_machine_mem_stochastic.txt " + PATH + "worst_fsm_st_" + str(k) + ".txt"
	  ...
	  file_fit_fsm_st.close()

	  #print "\n++++++++++++++  RESULTS Generation ",str(k) + " +++++++++++++++++++++++++++++\n"

          fit_average_fsm_st = fit_average_fsm_st / num_individuals
          value = str(fit_average_fsm_st)
          command = "echo " + value + " >> " + FILE_fitness_average_fsm_st
          #print "Command average: ", command
          status = os.system(command)
          print "Average generation FSM_st ",str(k)," ",str(fit_average_fsm_st)

          value = str(fit_worst_fsm_st)
          command = "echo " + value + " >> " + FILE_fitness_worst_fsm_st
          #print "Command worst: ", command
          status = os.system(command)
          print "Worst generation FSM_st ",str(index_worst_fsm_st)," ",str(fit_worst_fsm_st)

          value = str(fit_best_fsm_st)
          command = "echo " + value + " >> " + FILE_fitness_best_fsm_st
          #print "Command best: ", command
          status = os.system(command)
          print "Best generation FSM_st ",str(index_best_fsm_st)," ",str(fit_best_fsm_st)
	  ...
	  # It generates the next FSM_st generation
	  # It copies the fsm_st fitness file to a fitness file used by genetics_general
	  command = "cp " + FILE_fitness_fsm_st  + " " + FILE_fitness
          #print "cp fitness fsm_st  ", command
          status = os.system(command)

          #../genetics/genetics_general vasconcelos ../data/ 10 28980 avoid
          genetics_command = "../genetics/genetics_general vasconcelos "
          command = genetics_command + PATH + " " + str(num_individuals/2)+ " " + str(num_bits_individuals_fsm_st) + " " + population_name_fsm_st + " " + str(mutation_factor) + " " + str(cross_factor) + " " + str(num_best_factor) +  " > " + PATH + "genetics_command_fsm_st.dat"
          #command = genetics_command + PATH + " " + str(num_individuals/2)+ " " + str(num_bits_individuals_fsm_st) + " " + population_name_fsm_st + " " + str(mutation_factor) + " " + str(cross_factor) + " " + str(num_best_factor) 
          print "Command genetics next FSM_st generation: ", command
          status = os.system(command)
...
# here fix later the radio and file name (random)
	  ...
	  command = "cp " + PATH + "best_fsm_st_" + str(k) + ".txt " + PATH + "state_machine_mem_stochastic.txt"
	...
	if selection == str(6):
		...
		#command = robot_command + origin + destination + rest 
	#here
