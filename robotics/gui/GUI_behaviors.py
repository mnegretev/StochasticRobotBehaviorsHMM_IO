# GUI_behaviors.py 
'''
@author: Jesus Savage, UNAM-FI, 6-2023
...
# Probabiistic state machine constants
CNT1 = 5000.0
CNT2 = 1000.0
CNT3 = 0
...
#---------------------------------------------- Buttons Fields --------------------------------------------------------------------------------------------
                ...
                self.TestButtonMEALLYFSMGEN = tk.Button(self.topLevelWindow, width = 20, text = 'Test Meally FSM Gen.', bg = 'green', activebackground = 'green', command = self.toggleTestMeallyFSMGenBehaviors)
                self.TestButtonMOOREFSMGEN = tk.Button(self.topLevelWindow, width = 20, text = 'Test Moore FSM Gen.', bg = 'green', activebackground = 'green', command = self.toggleTestMooreFSMGenBehaviors)
                self.TestButtonPROBABILITIES = tk.Button(self.topLevelWindow, width = 20, text = 'Update Probabilities.', bg = 'green', activebackground = 'green', command = self.toggleProbabilityFile)


		# Plot Test generations button
                self.TestGenerationsButton = tk.Button(self.topLevelWindow, height = 4, width = 20, text = 'Test Generations', bg = 'green', activebackground = 'green', command = self.toggleTestGenerations)
                self.countTestGenerations = 0
		# Plot Evaluate button
                ...
                self.EvaluateButton = tk.Button(self.topLevelWindow, height = 4, width = 20, text = 'Genetic Algorithm', bg = 'green', activebackground = 'green', command = self.toggleGenetic)
		...
		# Check button flg_plot_vq
                self.var_plot_vq = IntVar()
                def command_plot_vq():
                        if self.var_plot_vq.get() == 0:
                                self.plot_vq.select()
                                self.var_plot_vq.set(1)
                        else:
                                self.plot_vq.deselect()
                                self.var_plot_vq.set(0)

                self.plot_vq = tk.Checkbutton(self.topLevelWindow, text="Show vq_sensors", variable= self.var_plot_vq,command=command_plot_vq)
                self.plot_vq.deselect()
                self.var_plot_vq.set(0)
...
#---------------------------------------- Values' fields -----------------------------------------------------------------------------
		...
		self.size_vq = tk.Entry(self.topLevelWindow, width = 4, foreground='white',background='black')
		...
		# Individual number
                ...
                self.num_ind = tk.Entry(self.topLevelWindow, width = 5, foreground='white',background='black')
...
#--------------------------- G R I D S ------------------------------------------------------------- 
		...
		self.label_noise_percentage_light_intensity.grid({'row':9, 'column': 5})        
		self.noise_percentage_light_intensity.grid({'row':9, 'column': 6})        
		self.label_noise_percentage_light_quadrant.grid({'row':9, 'column': 7})        
		self.noise_percentage_light_quadrant.grid({'row':9, 'column': 8})        
		self.label_movable.grid({'row':9, 'column': 0})        
		self.movable.grid({'row':9, 'column': 1})        
		...
		self.TestButtonMEALLYFSMGEN.grid({'row':4, 'column': 8})
		self.TestButtonMOOREFSMGEN.grid({'row':5, 'column': 8})
		self.TestButtonPROBABILITIES.grid({'row':7, 'column': 8})
		self.DemoButtonPath.grid({'row':8, 'column': 8})
		self.TestGenerationsButton.grid({'row':8, 'column': 7})
		self.plot_vq.grid({'row':9, 'column': 2})
...
#___________________________________________________________________________________________________ 
	...
	flg_plot_vq = self.var_plot_vq.get()
	number_steps = self.number_steps.get()
    ...
    def readResultFile(self,File_Results,File_Constants,num):
		...
		def __init__(self, File_Constants):
	      ...
	      #evaluation = Ct.K1*(number_steps_total - num_steps)*dif_o + Ct.K2*(dif_o_d-dif_d)*(number_steps_total - num_steps) + Ct.K3*dif_o_d/num_steps + Ct.K4*(number_steps_total - num_steps)/num_backwards + Ct.K5*(number_steps_total - num_steps)/num_turns + Ct.K6*(number_steps_total - num_steps)/num_stops + Ct.K7*num_advance*(number_steps_total - num_steps)
              ...
              #Ct.K4 = 1.0 #.10
	      #evaluation = Ct.K1*abs((number_steps_total - num_steps + 1)*dif_o) + (Ct.K2*abs(number_steps_total - num_steps + 1))/dif_d + Ct.K3*abs(dif_o_d/num_steps) + abs(number_steps_total - num_steps+1)/Ct.K4*num_backwards + Ct.K8*sd + Ct.K6*(number_steps_total - num_steps)/num_stops + Ct.K9*(number_steps_total - num_steps)/num_collisions
	      #print 'evaluation without K4 without dividing cnts ',str(evaluation)
	      ...
	      #print 'evaluation without K4 ',str(evaluation)
...
#+ Ct.K3*dif_o_d + Ct.K4*(number_steps_total - num_steps)/num_backwards + Ct.K5*(number_steps_total - num_steps)/num_turns + Ct.K6*(number_steps_total - num_steps)/num_stops + Ct.K7*num_advance*(number_steps_total - num_steps)
    ...
    def evaluate_hmm_state_machines(self,File_Name,num,BEHAVIOR):

	     indi_evaluation = 0
	     File_Input = File_Name + '_' + BEHAVIOR 
             #print 'Input HMM state machine ',File_Input

             for j in range(1, num+1):

		# First it simulates an HMM given a Lamda model
	     	#File_Output = PATH + File_Name + '_' + BEHAVIOR + '_' + str(j)
	     	File_Output = File_Name + '_' + BEHAVIOR + '_' + str(j)
             	#print 'Output HMM state machine ',File_Output
	    	#sim_command = "../hmm_robots/hmm_simulation/hmm_simulation -p " + PATH + " -f sim_state_machine -o " + File_Output + " -s 100 " + " > " + PATH + "test.dat"
	    	sim_command = "../hmm_robots/hmm_simulation/hmm_simulation -p " + PATH + " -f sim_state_machine_2 -o " + File_Output + " -s 100 " + " > " + PATH + "test.dat"
	    	#print "HMM generating command: ", sim_command
	    	status = os.system(sim_command)

		#It executes the probabilistic state machine
		File_test = PATH + "test_probabilistic.dat"
	    	command = "../hmm_robots/probabilistic_state_machine -p " + PATH + " -f hmm_fsm -o " + File_Output + " -e evaluation_state_machine " + " > " + File_test
	    	#print "Probabilistic command: ", command
	    	status = os.system(command)
		
		File_eval = PATH + "evaluation_state_machine.dat"
		file = open(File_eval, 'r')
		...
		for line in file:
                	...
                	#for i in range(0, len(words)):
                        	#print "words ""words ",words[i]
	

		#print "---------------------------------------"

                #dummy = file.readline()
		#print "Evaluation data ",dummy
                #words = dummy.split()
		#print "words ",words
		dif_states = float(words[0])
		dif_outputs = float(words[1])
		dif_prb = float(words[2])
                file.close()

                partial_evaluation = CNT1*dif_states + CNT2*dif_outputs + CNT3*dif_prb 
                #print str(j),'Partial evaluation ',partial_evaluation

                indi_evaluation = indi_evaluation + partial_evaluation


             indi_evaluation = indi_evaluation / num
             #indi_evaluation = 1 + (1/indi_evaluation)
             #print 'indi evaluation ',indi_evaluation
...
# Functions that handle the buttons
	...
	global number_unk
        ...
        self.largest.delete(0, END)
	self.largest.insert ( 0, '0.3' )
        largest = float(self.largest.get())

        self.path.delete(0, END)
	self.path.insert ( 0, '/home/biorobotica/data/data_19/' )
	PATH = self.path.get()
        File_Name = self.file.get()
	...
	#noise_percentage_advance = self.noise_percentage_advance.get()
	#print 'noise_percentage_advance ',str(noise_percentage_advance)
...
# Functions that handle the buttons
	...
	PATH = self.path.get()
        ...
        command = "cp ../../data/data_3/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_3/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
 ...
 # Test behavior FSM stochastic generate by a genetic algorithm
	...
	PATH = self.path.get()
        ...
        command = "cp ../../data/data_12/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_12/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	PATH = self.path.get()
        ...
        command = "cp ../../data/data_10/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_10/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	# Test behavior Input State Meally FSM generated by a genetic algorithm
    def toggleTestMeallyFSMGenBehaviors(self):
        global BEHAVIOR
        global BEST_BEHAVIOR
        global num_bh
        global st_out
	...
	#print INITIAL_PATH + " " +  ORIGINAL_PATH
        command = "cp ../../data/data_8/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_8/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)


        self.put_buttons_green()
        self.TestButtonMEALLYFSMGEN ['bg'] = 'red'
        self.TestButtonMEALLYFSMGEN['activebackground'] = 'red'
        C.update_idletasks()

        self.toggleTestBehaviors()


        # Test behavior Input State Moore FSM generated by a genetic algorithm
    def toggleTestMooreFSMGenBehaviors(self):
        global BEHAVIOR
        global BEST_BEHAVIOR
        global num_bh
        global st_out
	...
	#print INITIAL_PATH + " " +  ORIGINAL_PATH
	command = "cp ../../data/data_9/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

	command = "cp ../../data/data_9/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)


        self.put_buttons_green()
        self.TestButtonMOOREFSMGEN ['bg'] = 'red'
        self.TestButtonMOOREFSMGEN['activebackground'] = 'red'
        C.update_idletasks()
	...
	PATH = self.path.get()
        ...
        #print INITIAL_PATH + " " +  ORIGINAL_PATH
        command = "cp ../../data/data_13/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_13/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	command = "cp ../../data/data_4/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_4/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
   ...
   # Test behavior HMM generated by a genetic algorithm
	...
	if Meally == 1:
		...
		num_bh = 15
        	...
        	command = "cp ../../data/data_6/" + BEST_BEHAVIOR + " " + PATH
        	print "cp command: ", command
        	status = os.system(command)

        	command = "cp ../../data/data_6/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        	print "cp command: ", command
        	status = os.system(command)
   ...
   # Test behavior MDP generated by a genetic algorithm
	...
	PATH = self.path.get()
        ...
        command = "cp ../../data/data_7/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_7/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	BEHAVIOR = "nn"
        ...
        command = "cp ../../data/data_1/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_1/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	command = "cp ../../data/data_5/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_5/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	command = "cp ../../data/data_2/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_2/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	command = "cp ../../data/data_11/" + BEST_BEHAVIOR + " " + PATH
        print "cp command: ", command
        status = os.system(command)

        command = "cp ../../data/data_11/" + BEST_BEHAVIOR + " " + ORIGINAL_PATH
        print "cp command: ", command
        status = os.system(command)
	...
	noise_percentage_advance = float(self.noise_percentage_advance.get())
        #print 'noise_percentage_advance ',str(noise_percentage_advance)
	noise_percentage_angle = float(self.noise_percentage_angle.get())
        #print 'noise_percentage_angle ',str(noise_percentage_angle)
	noise_percentage_lidar = float(self.noise_percentage_lidar.get())
        #print 'noise_percentage_lidar ',str(noise_percentage_lidar)
	noise_percentage_lidar_sd = float(self.noise_percentage_lidar_sd.get())
        #print 'noise_percentage_lidar ',str(noise_percentage_lidar)
	noise_percentage_light_intensity = float(self.noise_percentage_light_intensity.get())
        #print 'noise_percentage_light_intensity ',str(noise_percentage_light_intensity)
	noise_percentage_light_quadrant = float(self.noise_percentage_light_quadrant.get())
        #print 'noise_percentage_light_quadrant ',str(noise_percentage_light_quadrant)


        # Noise Ranges
        y = noise_percentage_angle*max_angle_robot
        x = noise_percentage_advance*advance_robot
        z = noise_percentage_lidar*largest
        zdst = noise_percentage_lidar_sd*largest
        intensity = noise_percentage_light_intensity*50.
        theta = noise_percentage_light_quadrant*2*3.1416
        ...
        file_name_prb = 'random_settings_advance_angle_sensors.dat'
        FILE_PRB = PATH + file_name_prb
        print "Probability file in  ", FILE_PRB
        file_prb = open(FILE_PRB,'w')
        # Uniform error for advance
        #range_1 = 0.0
        range_1 = -x
    ...
    def toggleProbabilityFile(self):
    ...
    def toggleTestGenerations(self):
        global TESTING


        self.TestGenerationsButton['bg'] = 'red'
        self.TestGenerationsButton['activebackground'] = 'red'
        C.update_idletasks()
        self.put_buttons_green()


        print "Test Generations ", str(TESTING)
	
	self.CreateRandomFile()
	self.toggleEvaluateRobot()

	self.TestGenerationsButton['bg'] = 'green'
        self.TestGenerationsButton['activebackground'] = 'green'
        C.update_idletasks()
        self.put_buttons_green()



    def toggleGenetic(self):
        global TESTING
	...
	if num_behavior == 1:
		...
		num_bits_inputs = num_bits_symbols
		...
		print "num_bits_individuals MDP " + str(num_bits_individuals)
                ...
                # IN_FSM genetics' variables
                num_bits_symbols = int(math.log(size_vq)/math.log(2))
                #num_bits_inputs = num_bits_intensity +  num_bits_angle_light + num_bits_symbols;
                num_bits_inputs = num_bits_symbols;
		print "Num bits inputs ",str(num_bits_inputs)
                num_bits_memory_cells = 2*num_bits_inputs + 2*num_bits_states + 2*num_bits_outputs;
		print "Num bits memory cells ",str(num_bits_memory_cells)
                num_memory_cells = 2 << (num_bits_states -1)
                print "Num. Locationts ", str(num_memory_cells) 
                num_bits_individuals = num_memory_cells * num_bits_memory_cells;
                print "Num. bits individuals ", str(num_bits_individuals) 
                ...
                #num_symbols = 2 << (num_bits_inputs-1)
                num_symbols = size_vq
                num_variables= num_states_hmm * (1 + num_states_hmm + num_symbols + num_outputs) ;
                num_bits_individuals = num_bits_variables_hmm * num_variables;

	elif num_behavior == 13:

                # HMM State Machine genetics' variables
                num_bits_symbols = 2 
                num_bits_inputs = num_bits_symbols 
                num_symbols = 4
		num_states_hmm = 4
	...
	for k in range(total_num_generations, num_generations+total_num_generations):
	  ...
	  fit_best = 0.0
  	  ...
  	  # It creates the behavior fitness file
...
#here transform
	  ...
	  if num_behavior == 1:
		...
		# It transform the binary population file to an HMM Meally probability population files
		...
		command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_variables_hmm) + " " + str(num_states_hmm) + " " + str(num_symbols) + " " + str(num_outputs) + " " + population_name + " " + str(Meally) + " > " + PATH + "transform_hmm.dat "
		...
		# transform_gen mdp /home/savage/data/data_15/ 100 8 7 72 avoid
                ...
                # It transform the binary population file to a FSM population files
                # ../genetics/transform_gen state_machine ../data/ 20 4 3 5 3584 avoid_fsm
                genetics_command = "../genetics/transform_gen in_state_machine "
                command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_states) + " " + str(num_bits_outputs) + " " + str(num_bits_inputs) + " " + str(num_bits_individuals) + " " + population_name + " > " + PATH + "transform_in_fsm.dat"
                ...
                # It transform the binary population file to an HMM Moore probability population files
                #../genetics/transform_gen state_machine_hmm ../data/ 20 10 14 32 5 avoid
                genetics_command = "../genetics/transform_gen state_machine_hmm "
                command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_variables_hmm) + " " + str(num_states_hmm) + " " + str(num_symbols) + " " + str(num_outputs) + " " + population_name + " " + str(Meally) + " > " + PATH + "transform_hmm.dat "

	  elif num_behavior == 13:
                # It transform the binary population file to an HMM Moore state machine probability population files
                #../genetics/transform_gen state_machine_hmm ../data/ 20 10 14 32 5 avoid
                genetics_command = "../genetics/transform_gen state_machine_hmm "
                command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_variables_hmm) + " " + str(num_states_hmm) + " " + str(num_symbols) + " " + str(num_outputs) + " " + population_name + " " + str(Meally) + " > " + PATH + "transform_hmm_state_machine.dat "
	  ...
	  for l in range(0, num_individuals):
	     ...
	     if num_behavior == 13:
             	self.robot_command.insert ( 0,"../hmm_robots/probabilistic_state_machine")
	     else:
             	self.robot_command.insert ( 0,"../motion_planner/GoTo_State_Machine")
	     ...
	     number_unk = self.movable.get()
             ...
             # It copies the individual file to the file of the BEHAVIOR to be evaluated
	     ...
	     if num_behavior == 13:
             	...
             	#print "cp command: ", command
             	#status = os.system(command)
             	indi_evaluation=self.evaluate_hmm_state_machines(File_Name_cpy,100,BEHAVIOR)
             else:
...
# here fix later the radio and file name (random)
	...
	if selection == str(6):
		...
		command = robot_command + origin + destination + " -e " + File_Name + " -p " + PATH  + " -steps " + number_steps + " -noise " + str(flg_noise) + " -vq " + str(vq) + " -size_vq " + str(size_vq) + " -pr_out " + str(pr_out) + " -out_file " + File_Output + " -nn_unk " + str(number_unk) + " > test.dat" 
