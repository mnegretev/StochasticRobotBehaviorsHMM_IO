...
#HERE
	...
	def initial(self):
		...
		# Plot Training button
                self.TrainingButton = tk.Button(self.topLevelWindow, width = 20, text = 'Training / Use VQ ->', bg = 'green', activebackground = 'green', command = self.toggleTraining)
                self.countEvaluate = 0
		# Plot Robot button
		...
		self.path.insert ( 0, '../test_data_13/' )
		PATH = self.path.get()
      		# World's File entry 
...
#+ Ct.K3*dif_o_d + Ct.K4*(number_steps_total - num_steps)/num_backwards + Ct.K5*(number_steps_total - num_steps)/num_turns + Ct.K6*(number_steps_total - num_steps)/num_stops + Ct.K7*num_advance*(number_steps_total - num_steps)
    ...
    def evaluate_worlds(self,File_Name_cpy,num_worlds,flg_save,flg_vq):
...
# Functions that handle the buttons
	...
	num_centroids = self.size_vq.get()
        ...
        C.update_idletasks()
	indi_STATE_MACHINE_HUMAN=self.evaluate_worlds(File_Name_cpy,num_worlds,1,1)
...
#here vq
	...
	num_outputs_fsm = 8
	num_bits_outputs = int(math.log(num_outputs_fsm)/math.log(2))
	#print "num_bits_outputs ", str(num_bits_outputs)
	num_states_fsm = 16
	num_bits_states = int(math.log(num_states_fsm)/math.log(2))
	#print "num_bits_states ", str(num_bits_states)
	num_symbols = 2 ** size_vq
        num_bits_symbols = int(math.log(size_vq)/math.log(2))
	#print "num_bits_symbols ", str(num_bits_symbols)
	...
	#Reactive variables
        num_outputs_reactive = num_outputs_fsm
        num_bits_outputs_reactive = num_bits_outputs
        print "num_bits_outputs_reactive ", str(num_bits_outputs_reactive)
        num_symbols_reactive = num_symbols
        num_bits_symbols_reactive = num_bits_symbols
        print "num_bits_symbols_reactive ", str(num_bits_symbols_reactive)
        num_bits_intensity_reactive = num_bits_intensity
        num_bits_angle_light_reactive = num_bits_angle_light
        num_bits_inputs_reactive = num_bits_intensity_reactive + num_bits_symbols_reactive + num_bits_angle_light_reactive
        num_bits_memory_cells_reactive = num_bits_outputs_reactive;
        num_memory_cells_reactive = 2 << (num_bits_inputs_reactive -1)
        print "Num. Locationts reactive ", str(num_memory_cells_reactive) 
        num_bits_individuals_reactive = num_memory_cells_reactive * num_bits_memory_cells_reactive;
        print "num_bits_individuals_reactive ", str(num_bits_individuals_reactive) 
        population_name_reactive = "avoid_reactive"


	#Reactive variables stochastic
	num_bits_variables_stochastic = 8
        num_out_reactive_stochastic = 2 << (num_bits_outputs_reactive-1);
        num_bits_individuals_reactive_stochastic = num_memory_cells_reactive * num_out_reactive_stochastic * num_bits_variables_stochastic
        print "num_bits_individuals_reactive_stochastic ", str(num_bits_individuals_reactive_stochastic) 
        population_name_reactive_stochastic = "avoid_reactive_stochastic"
...
#checar esto despues
	...
	# It creates the files names for the REACTIVE were the average, best and word fitness are stored for generation
        FILE_fitness_average_reactive = PATH + 'fitness_average_reactive.dat'
        #print 'File_fitness_average_reactive ', FILE_fitness_average_reactive
        FILE_fitness_worst_reactive = PATH + 'fitness_worst_reactive.dat'
        #print 'File_fitness_worst_reactive ', FILE_fitness_worst_reactive
        FILE_fitness_best_reactive = PATH + 'fitness_best_reactive.dat'
        #print 'File_fitness_best_reactive ', FILE_fitness_best_reactive



	# It creates the files names for the stochastic REACTIVE were the average, best and word fitness are stored for generation
        FILE_fitness_average_reactive_stochastic = PATH + 'fitness_average_reactive_stochastic.dat'
        #print 'File_fitness_average_reactive_stochastic ', FILE_fitness_average_reactive_stochastic
        FILE_fitness_worst_reactive_stochastic = PATH + 'fitness_worst_reactive_stochastic.dat'
        #print 'File_fitness_worst_reactive_stochastic ', FILE_fitness_worst_reactive_stochastic
        FILE_fitness_best_reactive_stochastic = PATH + 'fitness_best_reactive_stochastic.dat'
        #print 'File_fitness_best_reactive_stochastic ', FILE_fitness_best_reactive_stochastic
	...
	#FILE_state_machine_NN = PATH + 'fitness_state_machine_nn.dat'
       	#print 'File_state_machine_human ', FILE_state_machine_human
	#FILE_MDP = PATH + 'fitness_MDP.dat'
        #print 'FILE_MDP ', FILE_MDP
	#FILE_MDP_EIGHT = PATH + 'fitness_MDP_eight.dat'
        #print 'FILE_MDP ', FILE_MDP
...
#here
		...
		# it generates the first population for the REACTIVE individuals
                genetics_command = "../genetics/genetics_general genera vasconcelos "
                command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_individuals_reactive) + " " + population_name_reactive + " > " + PATH + "genetics_general_REACTIVE.dat"
                print "First generation command genetics REACTIVE: ", command
                status = os.system(command)


		# it generates the first population for the stochastic REACTIVE individuals
                genetics_command = "../genetics/genetics_general genera vasconcelos "
                command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_individuals_reactive_stochastic) + " " + population_name_reactive_stochastic + " > " + PATH + "genetics_general_REACTIVE_stochastic.dat"
                print "First generation command genetics REACTIVE_stochastic: ", command
                status = os.system(command)
		...
		# REACTIVE Files
                command = "rm " + FILE_fitness_average_reactive
                #print "Command delete: ", command
                status = os.system(command)
                command = "rm " + FILE_fitness_worst_reactive
                #print "Command delete: ", command
                status = os.system(command)
                command = "rm " + FILE_fitness_best_reactive
                #print "Command delete: ", command
                status = os.system(command)

		# REACTIVE Files
                command = "rm " + FILE_fitness_average_reactive_stochastic
                #print "Command delete: ", command
                status = os.system(command)
                command = "rm " + FILE_fitness_worst_reactive_stochastic
                #print "Command delete: ", command
                status = os.system(command)
                command = "rm " + FILE_fitness_best_reactive_stochastic
                #print "Command delete: ", command
                status = os.system(command)
...
#here
	...
	for k in range(total_num_generations, num_generations+total_num_generations):
	  ...
	  fit_average_reactive = 0.0
	  fit_worst_reactive = 1000000.0
	  fit_best_reactive = 0.0

	  fit_average_reactive_stochastic = 0.0
	  fit_worst_reactive_stochastic = 1000000.0
	  fit_best_reactive_stochastic = 0.0
	  ...
	  #fit_best_nn = 0.0
	   ...
	   # It creates the reactive fitness file
          FILE_fitness_reactive = PATH + 'fitness_reactive.dat'
          file_fit_reactive = open(FILE_fitness_reactive,'w')
          #print 'File_fitness_reactive ', FILE_fitness_reactive


	  # It creates the reactive stochastic fitness file
          FILE_fitness_reactive_stochastic = PATH + 'fitness_reactive_stochastic.dat'
          file_fit_reactive_stochastic = open(FILE_fitness_reactive_stochastic,'w')
          #print 'File_fitness_reactive_stochastic ', FILE_fitness_reactive_stochastic
...
#here
	...
	# It transform the binary population file to a REACTIVE population files
 	  #transform_gen reactive ../test_data_10/ 100 3 5 avoid_reactive
          ...
          command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_outputs_reactive) + " " + str(num_bits_inputs_reactive) + " " +  population_name_reactive + " > " + PATH + "transform_reactive.dat"
#here
          print "Command transform REACTIVE: ", command
          status = os.system(command)
	...
	# It transform the binary population file to a REACTIVE stochastic population files
	  #transform_gen reactive_stochastic ../test_data_8/ 100 3 8 avoid_stochastic 8
          ...
          command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_outputs_reactive) + " " + str(num_bits_inputs_reactive) + " " +  population_name_reactive_stochastic + " " + str(num_bits_variables_stochastic) + " > " + PATH + "transform_reactive_stochastic.dat"
#here
          print "Command transform REACTIVE_stochastic: ", command
          status = os.system(command)
	  ...
	  # it copies the human made state machine state_machine_obstacle_destination.txt
          ...
          indi_STATE_MACHINE_HUMAN=self.evaluate_worlds(File_Name_cpy,num_worlds,1,0)
	  ...
	  # It evaluates the MDP
          #%self.robot_command.delete(0, END)
	  #%elf.robot_command.insert ( 0,"../mdp_navigation/GoTo_MDP")
          #%self.selection.delete(0, END)
          #%self.selection.insert ( 0, '4' )
	  #%self.vq.delete(0, END)
          #%self.vq.insert ( 0, '0' )
	  ...
	  #self.pr_out.insert ( 0, '0' )
          ...
          #%indi_MDP=self.evaluate_worlds(File_Name_cpy,num_worlds,1,0)
          #num_trn= num_training
          #%value = str(indi_MDP)
          #%command = "echo " + value + " >> " + FILE_MDP
          #print "Command MDP evaluate: ", command
          #%status = os.system(command)
          # It copies the last *.raw  and *wrl files of this evaluation
          #command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.wrl '  + PATH + "mdp_" + str(k) + ".wrl"
          #print "cp command MDP world: ", command
          #status = os.system(command)
          #command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.raw '  + PATH + "mdp_" + str(k) + ".raw"
          #print "cp command MDP raw: ", command
          #%status = os.system(command)
          #%print "Evaluation MDP ",str(indi_MDP)


	  # It evaluates the MDP with 8 sensors
          #%self.robot_command.delete(0, END)
          #%self.robot_command.insert ( 0,"../mdp_navigation/GoTo_MDP")
	  
	  #%previous_selection = self.selection.get()
          #%self.selection.delete(0, END)
          #%self.selection.insert ( 0, '6' )
	  #%self.vq.delete(0, END)
          #%self.vq.insert ( 0, '0' )
	  #self.pr_out.delete(0, END)
	  #self.pr_out.insert ( 0, '0' )
          ...
          #%indi_MDP_8=self.evaluate_worlds(File_Name_cpy,num_worlds,1,0)
          #%num_trn= num_training
          #%value = str(indi_MDP_8)
          #%command = "echo " + value + " >> " + FILE_MDP_EIGHT
          #print "Command MDP evaluate: ", command
          #%status = os.system(command)
          # It copies the last *.raw  and *wrl files of this evaluation
          #%command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.wrl '  + PATH + "mdp8_" + str(k) + ".wrl"
          #print "cp command MDP world: ", command
          #status = os.system(command)
          #%command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.raw '  + PATH + "mdp8_" + str(k) + ".raw"
          #print "cp command MDP raw: ", command
          #%status = os.system(command)
          #%print "Evaluation MDP EIGHT ",str(indi_MDP_8)

          #%self.selection.delete(0, END)
          #%self.selection.insert ( 0, previous_selection)
          #%C.update_idletasks()
	  ...
	  #C.update_idletasks()
          ...
          #print "cp command: ", command
          #%status = os.system(command)
	  #%indi_HMM_HUMAN=self.evaluate_worlds(File_Name_cpy,num_worlds,0,0)
	  ...
	  # it evaluates each of HMM, FSM, REACTIVE and NN individuals
	  for l in range(0, num_individuals):
	     ...
	     #cp hmm_avoid_0.dat hmm_destination_avoidance_fsm.prb
             ...
             #print "cp command: ", command
             #%status = os.system(command)
	     #%indi_evaluation=self.evaluate_worlds(File_Name_cpy,num_worlds,0,0)
	     ...
	     # it evaluates the REACTIVE
       	     self.robot_command.delete(0, END)
             self.robot_command.insert ( 0,"../motion_planner/GoTo_State_Machine")
       	     self.selection.delete(0, END)
	     self.selection.insert ( 0, '8' )
	     self.vq.delete(0, END)
	     self.vq.insert ( 0, '1' )
	     self.pr_out.delete(0, END)
	     self.pr_out.insert ( 0, '0' )
             num_centroids = self.size_vq.get()
	     index_best_reactive = 1

             C.update_idletasks()

	     # It copies the individual reactive file to the reactive table file
             command = "cp " + PATH + population_name_reactive + "_" + str(l) + ".dat " + PATH + "reactive_table_vq.txt"
             #print "cp command: ", command
             status = os.system(command)
             indi_evaluation=self.evaluate_worlds(File_Name_cpy,num_worlds,0,0)
             value = str(indi_evaluation) + '\n'
             file_fit_reactive.write(value)
	     #print "Evaluation Gen. ",str(k)," ind. REACTIVE ",str(l)," = ",str(indi_evaluation)
             fit_average_reactive = fit_average_reactive + indi_evaluation
             if(indi_evaluation > fit_best_reactive):
                 fit_best_reactive = indi_evaluation
                 index_best_reactive = l
                 # it saves the information of the best individual: environment, the run and the genoma
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.wrl '  + PATH + "best_reactive_" + str(k) + ".wrl"
                 #print "cp command best REACTIVE world: ", command
                 #status = os.system(command)
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.raw '  + PATH + "best_reactive_" + str(k) + ".raw"
                 #print "cp command best  REACTIVE raw: ", command
                 status = os.system(command)
                 command = "cp " + PATH + "reactive_table_vq.txt " + PATH + "best_reactive_table_vq_" + str(k) + ".txt"
                 #print "cp command best_REACTIVE behaviour: ", command
                 status = os.system(command)
             if(indi_evaluation < fit_worst_reactive):
                 fit_worst_reactive = indi_evaluation
                 index_worst_reactive = l
                 # it saves the information of the worst individual: environment, the run and the genoma
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.wrl '  + PATH + "worst_reactive_" + str(k) + ".wrl"
                 #print "cp command worst_FSM world: ", command
                 #status = os.system(command)
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.raw '  + PATH + "worst_reactive_" + str(k) + ".raw"
                 #print "cp command worst raw: ", command
                 status = os.system(command)
                 command = "cp " + PATH + "reactive_table_vq.txt " + PATH + "worst_reactive_table_vq_" + str(k) + ".txt"
                 #print "cp command worst behaviour: ", command
                 status = os.system(command)


	     # it evaluates the REACTIVE stochastic
             self.robot_command.delete(0, END)
             self.robot_command.insert ( 0,"../motion_planner/GoTo_State_Machine")
             self.selection.delete(0, END)
             self.selection.insert ( 0, '8' )
             ...
             self.vq.insert ( 0, '1' )
             self.pr_out.delete(0, END)
             self.pr_out.insert ( 0, '1' )
             num_centroids = self.size_vq.get()
             index_best_reactive_stochastic = 1

             C.update_idletasks()

             # It copies the individual reactive stochasti file to the reactive stochastic probability file
             command = "cp " + PATH + population_name_reactive_stochastic + "_" + str(l) + ".dat " + PATH + "stochastic_reactive_table_vq.prb"
             #print "cp command: ", command
             status = os.system(command)
             indi_evaluation=self.evaluate_worlds(File_Name_cpy,num_worlds,0,0)
             value = str(indi_evaluation) + '\n'
             file_fit_reactive_stochastic.write(value)
             #print "Evaluation Gen. ",str(k)," ind. REACTIVE stochastic ",str(l)," = ",str(indi_evaluation)
             fit_average_reactive_stochastic = fit_average_reactive_stochastic + indi_evaluation
             if(indi_evaluation > fit_best_reactive_stochastic):
                 fit_best_reactive_stochastic = indi_evaluation
                 index_best_reactive_stochastic = l
                 # it saves the information of the best individual: environment, the run and the genoma
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.wrl '  + PATH + "best_reactive_stochastic_" + str(k) + ".wrl"
                 #print "cp command best REACTIVE stochastic world: ", command
                 #status = os.system(command)
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.raw '  + PATH + "best_reactive_stochastic_" + str(k) + ".raw"
                 #print "cp command best  REACTIVE raw: ", command
                 status = os.system(command)
                 command = "cp " + PATH + "stochastic_reactive_table_vq.prb " + PATH + "best_stochastic_reactive_table_vq_" + str(k) + ".prb"
                 #print "cp command best_REACTIVE stochastic behaviour: ", command
                 status = os.system(command)
             if(indi_evaluation < fit_worst_reactive_stochastic):
                 fit_worst_reactive_stochastic = indi_evaluation
                 index_worst_reactive_stochastic = l
                 # it saves the information of the wors individual: environment, the run and the genoma
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.wrl '  + PATH + "worst_reactive_stochastic_" + str(k) + ".wrl"
                 #print "cp command worst_reactive_stochastic world: ", command
                 #status = os.system(command)
                 command = "cp " + PATH + File_Name_cpy + "_"+ str(num_worlds) + '.raw '  + PATH + "worst_reactive_stochastic_" + str(k) + ".raw"
                 #print "cp command worst raw: ", command
                 status = os.system(command)
                 command = "cp " + PATH + "stochastic_reactive_table_vq.prb " + PATH + "worst_reactive_stochastic_vq_" + str(k) + ".prb"
                 #print "cp command worst reactive stochastic behaviour: ", command
                 status = os.system(command)
	  ...
	  #%file_fit.close()
	  file_fit_fsm.close()
	  file_fit_reactive.close()
	  file_fit_reactive_stochastic.close()
	  #%file_fit_nn.close()
	  ...
	  #print "MDP " + str(indi_MDP)
	  #print "MDP_8 " + str(indi_MDP_8)
	  ...
	  fit_average_reactive = fit_average_reactive / num_individuals
          value = str(fit_average_reactive)
          command = "echo " + value + " >> " + FILE_fitness_average_reactive
          #print "Command average: ", command
          status = os.system(command)
          print "Average generation REACTIVE ",str(k)," ",str(fit_average_reactive)

          value = str(fit_worst_reactive)
          command = "echo " + value + " >> " + FILE_fitness_worst_reactive
          #print "Command worst: ", command
          status = os.system(command)
          print "Worst generation REACTIVE ",str(index_worst_reactive)," ",str(fit_worst_reactive)

          value = str(fit_best_reactive)
          command = "echo " + value + " >> " + FILE_fitness_best_reactive
          #print "Command best: ", command
          status = os.system(command)
          print "Best generation REACTIVE ",str(index_best_reactive)," ",str(fit_best_reactive)


          fit_average_reactive_stochastic = fit_average_reactive_stochastic / num_individuals
          value = str(fit_average_reactive_stochastic)
          command = "echo " + value + " >> " + FILE_fitness_average_reactive_stochastic
          #print "Command average: ", command
          status = os.system(command)
          print "Average generation REACTIVE stochastic ",str(k)," ",str(fit_average_reactive_stochastic)

          value = str(fit_worst_reactive_stochastic)
          command = "echo " + value + " >> " + FILE_fitness_worst_reactive_stochastic
          #print "Command worst: ", command
          status = os.system(command)
          print "Worst generation REACTIVE stochastic",str(index_worst_reactive_stochastic)," ",str(fit_worst_reactive_stochastic)

          value = str(fit_best_reactive_stochastic)
          command = "echo " + value + " >> " + FILE_fitness_best_reactive_stochastic
          #print "Command best: ", command
          status = os.system(command)
          print "Best generation REACTIVE stochastic ",str(index_best_reactive_stochastic)," ",str(fit_best_reactive_stochastic)
	  ...
	  # It generates the next REACTIVE generation
          # It copies the reactive fitness file to a fitness file used by genetics_general
          command = "cp " + FILE_fitness_reactive  + " " + FILE_fitness
          #print 1"cp fitness reactive  ", command
          status = os.system(command)

          genetics_command = "../genetics/genetics_general vasconcelos "
          command = genetics_command + PATH + " " + str(num_individuals/2)+ " " + str(num_bits_individuals_reactive) + " " + population_name_reactive + " " + str(mutation_factor) + " " + str(cross_factor) + " " + str(num_best_factor) +  " > " + PATH + "genetics_command_reactive.dat"
          print "Command genetics next REACTIVE generation: ", command
          status = os.system(command)




	  # It generates the next stochastic REACTIVE generation
          # It copies the reactive fitness file to a fitness file used by genetics_general
          command = "cp " + FILE_fitness_reactive_stochastic  + " " + FILE_fitness
          #print 1"cp fitness reactive  ", command
          status = os.system(command)

          genetics_command = "../genetics/genetics_general vasconcelos "
          command = genetics_command + PATH + " " + str(num_individuals/2)+ " " + str(num_bits_individuals_reactive_stochastic) + " " + population_name_reactive_stochastic + " " + str(mutation_factor) + " " + str(cross_factor) + " " + str(num_best_factor) +  " > " + PATH + "genetics_command_reactive_stochastic.dat"
          print "Command genetics next REACTIVE generation: ", command
          status = os.system(command)
...
# here fix later the radio and file name (random)
	  ...
	  command = "cp " + PATH + "best_fsm_" + str(k) + ".txt " + PATH + "state_machine_mem_" + str(size_vq) + ".txt"
	  #print "Command cp best FSM: ", command
          status = os.system(command)

	  command = "cp " + PATH + "best_reactive_table_vq_" + str(k) + ".txt " + PATH + "reactive_table_vq.txt" 
	  #print "Command cp best reactive: ", command
          status = os.system(command)
	...
	if selection == str(6):
		...
		command = robot_command + origin + destination + " -e " + File_Name + " -p " + PATH  + " -steps " + number_steps + " -noise " + str(flg_noise) + " -vq " + str(vq) + " -size_vq " + str(size_vq) + " -pr_out " + str(pr_out) + " > test.dat "
		#print "MDP Command: ", command
	else:
		robot_command = self.robot_command.get()
		origin = " -x " + str(mouse_1_x) + " -y " + str(mouse_1_y) + " -a " + str(angle_robot)
		destination = " -v " + str(mouse_3_x) + " -z " + str(mouse_3_y)
		rest = " -s " + sensor + " -n " + num_sensors + " -t " + origen_angle + " -r " + range_angle + " -radio " + str(radio_robot) + " -advance " + str(advance_robot) + " -MaxAngle " + str(max_angle_robot) + " -steps " + number_steps + " -selection " + selection + " -largest " + largest + " -p " + PATH + " -e " + File_Name + " -noise " + str(flg_noise) + " -vq " + str(vq) + " -size_vq " + str(size_vq) + " -pr_out " + str(pr_out) 
