...
#HERE
	...
	def initial(self):
		...
		self.label_robot_command = tk.Label(self.topLevelWindow,text =  "Robot's command")
        	self.robot_command = tk.Entry(self.topLevelWindow, width = 30, foreground='white',background='black')
		...
		def __init__(self, File_Constants):
	      ...
	      for line in file_results:
			...
			if len(words) > 1:
					...
					if(angle != 0.0 and previous_angle != 0.0):
						num_turns=num_turns + 1

					if(advance > 0.0 and previous_advance > 0.0):
	      ...
	      evaluation = Ct.K1*(dif_o) + Ct.K2/(dif_d) + Ct.K3*dif_o_d/num_steps + Ct.K4*num_steps/num_backwards + Ct.K5*num_steps/num_turns + Ct.K6*num_steps/num_stops + Ct.K7*num_advance/num_steps
...
# Functions that handle the buttons
	...
	# It creates the files names were the average, best and word fitness are stored for generation
...
#here
  	  ...
  	  # It creates the fitness file
          FILE_fitness = PATH + 'fitness.dat'
          file_fit = open(FILE_fitness,'w')
          #print 'File_fitness ', FILE_fitness


	  # It transform the binary population file to a probability population files
	  ...
	  # It copies the last *.raw  and *wrl files of this evaluation
          ...
          status = os.system(command)
	  print "Evaluation HMM ",str(indi_HMM_HUMAN)
	  ...
	  # It evaluates the Viterbi created probability
          ...
          self.robot_command.insert ( 0,"../hmm_robots/GoTo_State_Machine_HMM")
          C.update_idletasks()
	  command = "../hmm_robots/Get_HMM -p " + PATH + " -f  training_random -n " + str(num_trn) + " -m " + name_viterbi
	  ...
	  # it evaluates each of HMM individuals
	  for l in range(0, num_individuals):
	     ...
	     print "Evaluation individual ",str(l)," ",str(indi_evaluation)
             #file_evl.close()
	  ...
	  print "FSM " + str(indi_STATE_MACHINE_HUMAN)
	  ...
	  #../genetics/genetics_general vasconcelos ../data/ 10 28980 avoid
          ...
          command = genetics_command + PATH + " " + str(num_individuals/2)+ " " + str(num_bits_individuals) + " " + population_name + " > " + PATH + "genetics_command.dat" 
