...
#___________________________________________________________________________________________________ 
	...
	global radio_robot
        ...
        #print "X1 ", X1, " Y1 ", Y1
        #print "X2 ", X2, " Y2 ", Y2
        oval = C.create_oval(X1,Y1,X2,Y2, outline="black",fill="blue", width=1)
...
#here
	...
	num_inputs = num_inputs_line 
	num_bits_inputs = 4
	#num_inputs_sensors = num_inputs * num_bits_inputs
	num_inputs_sensors = num_inputs
	num_outputs = num_inputs_sensors
	num_nn_layer = range(10)
	num_nn_layer[0] = num_inputs_sensors * ( 1 + num_inputs_delayed) + num_recurrent
	num_nn_layer[1] = 40
	num_nn_layer[2] = 20
	num_nn_layer[3] = num_outputs + num_recurrent  
	num_steps = 8
	...
	if new_generation == 1:
		...
		#genetics/genetics_general genera vasconcelos test_data/ 100 8032 sort_nn > test_data/genetics_general_NN.dat
		...
		total_num_generations = 0
        	...
        	input_file = PATH + "input_file_0.dat"
        	file_inputs = open(input_file,'w')
        	s = str(num_lines) + '\n'
        	file_inputs.write(s)
        	s = str(num_inputs_line) + '\n'
        	file_inputs.write(s)
        	s = str(num_bits_number) + '\n'
        	file_inputs.write(s)
        	for x in range(num_lines):
              		for y in range(num_inputs_line):
                        	n_ran = random.randint(0,9)
                        	s = str(n_ran) + ' '
                       		file_inputs.write(s)
              		s = '\n'
              		file_inputs.write(s)
	...
	for k in range(total_num_generations, num_generations+total_num_generations):
	  ...
	  fit_best_nn = 0.0
  	  ...
  	  # It creates the NN fitness file
          ...
          FILE_results = PATH + 'results.dat'

	  # It transform the binary population file to a NN population files
	  # genetics/transform_gen neural_nets test_data/ 100 8 7 avoid_nn 2 4 0 20 20 20 > test_data/transform_nn.dat
          genetics_command = "genetics/transform_gen neural_nets "
          command = genetics_command + PATH + " " + str(num_individuals) + " " + str(num_bits_variables_nn) + " " + str(num_bits_variables_nn_float) + " " +  population_name_nn + " " + str(num_layers) + " " + str(num_recurrent) + " " + str(num_inputs_delayed) + " " + str(num_inputs_sensors) + " " + str(num_nn_layer[1]) + " " + str(num_nn_layer[2]) + " " + str(num_outputs) + " > " + PATH + "transform_nn.dat"
          #print "Command transform NN: ", command
          status = os.system(command)

          self.robot_command.delete(0, END)
          self.robot_command.insert ( 0,"Sort_State_Machine_NN")
          self.selection.delete(0, END)
          self.selection.insert ( 0, '3' )
          C.update_idletasks()


	  input_file = PATH + "input_file_" + str(k) + ".dat"
	  file_inputs = open(input_file,'w')
	  s = str(num_lines) + '\n'
          file_inputs.write(s)
	  s = str(num_inputs_line) + '\n'
	  ...
	  #here
	  #input_file = "input_file_" + str(k)
	  input_file = "input_file_0"

	  # it evaluates each of the NN individuals
	  for l in range(0, num_individuals):
	     #print "Individual ",str(l)

             # It copies the individual nn file to the NN file used by Sort_State_Machine_NN
             command = "cp " + PATH + "nn_" + population_name_nn + "_" + str(l) + ".dat " + PATH + "rnn_fsm.txt"
             #print "cp command NN: ", command
             status = os.system(command)

	     output_nn = "output_nn_" + str(l)
	     sort_command = "Sort_State_Machine_NN -p test_data/ -f " + input_file + " -o " + output_nn + " -steps " + str(num_steps) + " -nn rnn_fsm -selection 3 > command_sort.txt"
             #print "NN Sort Command: ", sort_command
             status = os.system(sort_command)

	     #./evaluate_sort -p test_data/ -f input_file -o output_file
	     evaluate_sort_command = "evaluate_sort -p test_data/ -f " + input_file + " -o " + output_nn + " > command_evaluate.txt"
	     #print "NN Evaluate Command: ", evaluate_sort_command
             status = os.system(evaluate_sort_command)
             file_results = open(FILE_results,'r')
             indi_evaluation = 1.0 / float(file_results.readline())
	     file_results.close()
	     ...
	     #print "Evaluation Gen. ",str(k)," ind. NN ",str(l)," = ",str(indi_evaluation)
             ...
             if(indi_evaluation > fit_best_nn):
                 ...
                 #print "cp command best NN: ", command
                 status = os.system(command)
             ...
             if(indi_evaluation < fit_worst_nn):
                 ...
                 #print "cp command worst NN: ", command
                 status = os.system(command)
