...
#here
	...
	for k in range(total_num_generations, num_generations+total_num_generations):
	  ...
	  for l in range(0, num_individuals):
	     ...
	     sort_command = "Sort_State_Machine_NN -p test_data/ -f " + input_file + " -o " + output_nn + " -steps " + str(num_steps) + " -nn rnn_fsm -selection 3"
             print "NN Sort Command: ", sort_command
             status = os.system(sort_command)

	     #./evaluate_sort -p test_data/ -f input_file -o output_file
	     evaluate_sort_command = "evaluate_sort -p test_data/ -f " + input_file + " -o " + output_nn
	     print "NN Sort Command: ", evaluate_sort_command
