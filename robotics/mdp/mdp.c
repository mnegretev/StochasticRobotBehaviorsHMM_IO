...
int absr(int row,int column,int rows,int columns,int a,int k,int l){
    ...
    switch(a){
        case UP:
            // simple case, same direction as definition
            z = row+k-1;
            break;
        case RIGHT:
            z = row-l+1;
...
int absc(int row,int column,int rows,int columns,int a,int k,int l){
    ...
    return z>=0?(z<columns?z:columns-1):0;
}
...
void wire_map(S_MDP *mdp,map *m,double **probs){
    ...
    for(int i=0;i<m->rows;i++){
        for(int j=0;j<m->columns;j++){
            ...
            //actions
            for(int a=UP;a<=LEFT;a++){ 
                // explore Moore's neighborhood
                for(int k=0;k<3;k++){
                    for(int l=0;l<3;l++){
                        prob = probs[absr(1,1,3,3,a,k,l)][absc(1,1,3,3,a,k,l)];
