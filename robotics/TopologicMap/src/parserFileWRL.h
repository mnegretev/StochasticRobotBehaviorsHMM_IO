...
std::vector<Polygon> parserFile(std::string file, float &w, float &h) {
					...
					if (nameLine == "dimensions") {
						...
						do {
							...
							if (xstr == "" || ystr == "")
								...
								w = atof(xstr.c_str()) * 10.0;
								h = atof(ystr.c_str()) * 10.0;
							}
						} while (flag);
						...
						// TODO Verify why this condition uncomment to save a walls grown polygons
						/*if(typePolygon != WALL)
